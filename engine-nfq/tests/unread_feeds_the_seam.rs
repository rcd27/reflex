//! У ДВЕРИ ШВА ПОЯВЛЯЕТСЯ КОРМЯЩИЙ.
//!
//! Дверь ([`reflex_core::detector::Sensed::Unread`]) и шов, который её принимает
//! ([`reflex_core::interleave::Interleave::unread`], [`reflex_runtime::timed::on_grid`]), были
//! построены раньше и проверены на выдуманных значениях. Здесь у двери впервые стоит НАСТОЯЩИЙ
//! разбор провода — `reflex_engine_nfq::parse::framed` — и демонстрируется способность целиком,
//! от байтов до показания цепочки, одним прогоном.
//!
...
/// ПОСТРОЕНИЕ TCP-КАДРА — способ дословно взят из `engine-nfq/tests/parse.rs`: тот же порядок
/// полей, те же смещения. Свой способ здесь не изобретается, потому что кадр, собранный иначе, не
/// доказывал бы ничего о разборе, которым живёт очередь.
fn frame(
    src_ip: u32,
    dst_ip: u32,
    src_port: u16,
