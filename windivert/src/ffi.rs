...
//!
//! ГЕЙТ ПО МОДУЛЮ (`lib.rs`) СТОИТ НА `carrier`, А НЕ НА ЭТОМ ФАЙЛЕ ЦЕЛИКОМ: `extern "system"`
//! блоки ниже линковать не с чем нигде, кроме Windows, и помечены `#[cfg(windows)]` сами; но
//! раскладка `WINDIVERT_ADDRESS` — ЧИСТЫЕ ДАННЫЕ (`#[repr(C)]`, ноль unsafe, ноль линковки), и
//! оставлена вне гейта нарочно: арифметика битового поля — единственное в крейте, что реально
//! БЕЖИТ здесь, а не только собирается под чужой таргет, и прятать её за гейтом заодно с FFI
//! значило бы отдать последний живой прогон.
//!
//! ОТКУДА ВЗЯТО — координаты источников; что из этого догадано, а не выверено, названо в докблоке
//! крейта:
...
mod tests {
    ...
    /// молча на живом драйвере (флаг встал бы не туда) и НЕ покраснела бы под `cargo check` (тип тот же,
