...
/// а не константным `None` в нём — заглушка, запрещённая §9.1. Прогон, предъявляющий этот отказ
/// (`compile_fail` + положительная пара), стоит на заместителе — [`crate::witness`], и почему на
/// заместителе, сказано там.
    ...
    fn attempt_recv(&mut self, until: Instant) -> RecvOutcome {
        ...
        let got = if ok == ffi::TRUE {
            ...
            match unsafe { ffi::WaitForSingleObject(event, Self::millis_until(until)) } {
                ...
                // функции про `CancelIoEx`). Оба исхода названы ЗДЕСЬ, а не армом
                // `ffi::WAIT_TIMEOUT | _`: перечислять константу рядом с `_`, который её и так
                // забирает, значит обещать разбор по имени, которого нет, — а имя из арма прочтёт
                // тот, кто станет их различать, и решит, что различение уже написано.
                //
...
///
/// ЭТО ДИСЦИПЛИНА СТРУКТУРЫ, НЕ ГАРАНТИЯ КОМПИЛЯТОРА — названо прямо, потому что замерено:
/// третья ветка `RecvOutcome`, дописанная с РАННИМ `return` в обход хвостового `sleep` (тем самым
/// способом, каким закон и нарушается), собирается под кросс-проверкой без единого предупреждения.
/// Значит следующая безответная ветка обязана возвращать ЗНАЧЕНИЕМ — обязанность автора, не то,
/// что поймает здесь хоть что-нибудь автоматическое.
...
/// Рецепт носителя WinDivert: `impl reflex::IntoCarrier` ниже — та же дверь `engine(..)`, что и у
/// `Nfqueue` (`reflex/src/nfqueue.rs`), не совпавшая по сигнатурам копия.
...
impl WinDivert {
    ...
    ///
    /// ЦЕПОЧКА ПОТРЕБИТЕЛЯ ЖИВЁТ ОДНИМ МЕСТОМ, и это не доктест: `examples/detect-silent-drop/`
    /// `src/bin/windows.rs`. Бинарь кросс-СОБИРАЕТСЯ (`cargo check -p detect-silent-drop --target
    /// x86_64-pc-windows-msvc`) и краснеет от мутанта (снять `.extract` — две ошибки), доктест же
    /// не собрался бы ни там, ни здесь. Копия цепочки в докблоке была бы вторым её описанием,
    /// расходящимся с первым молча; от `Nfqueue` она отличается ровно первой строкой.
...
impl IntoCarrier for WinDivert {
    ...
    /// `Local` — потому что у WinDivert нет ядерного дома: край и состояние он строит сам, в
    /// юзерспейсе, той же формой, какой `reflex::LocalNfqueue` строит их для очереди без conntrack.
    ...
    /// Раскладка — УМОЛЧАНИЕ ОБЩЕГО ДОМА, не своё: марка есть предмет одной сущности независимо от
    /// того, где она лежит физически (ядерный `ct_mark` против юзерспейс-карты `Local`). Копия
    /// чисел здесь развела бы тег двух носителей молча, и `Distress::Diverged` заговорил бы на
    /// ровном месте.
