...
impl WinDivert {
    ...
    /// прогон не поставлен.
    ///
    /// **ПРОГОН С ТЕХ ПОР ПОСТАВЛЕН, и не здесь** (10.09.2026). Ту же цепочку несёт бинарь-витрина
    /// `examples/detect-silent-drop/src/bin/windows.rs`, а бинарь — не доктест: его кросс-сборка
    /// СОБИРАЕТ. `cargo check -p detect-silent-drop --target x86_64-pc-windows-msvc` → `Finished`,
    /// и способность замера краснеть доказана мутантом (убрать `.extract` — две ошибки).
    ///
    /// Предел этого доктеста остаётся в силе, снят предел УТВЕРЖДЕНИЯ: форма годна по замеру, а не
    /// по обещанию. Работает ли она — скажет первый живой прогон, которого по-прежнему нет.
    ///
    /// ```ignore
    /// use reflex::*;
...
impl IntoCarrier for WinDivert {
    ...
    /// Раскладка — УМОЛЧАНИЕ ОБЩЕГО ДОМА, не своё. У WinDivert нет ядерного `ct_mark`, но `Local`
    /// (докблок крейта) кодирует состояние детектора В ТОМ ЖЕ формате бит, независимо от того, где
    /// марка хранится физически (ядро против юзерспейс-карты) — раскладка есть предмет ОДНОЙ
    /// сущности. Прежде эти два числа стояли здесь второй копией рядом с `reflex/src/nfqueue.rs`, и
    /// докблок копию признавал; разойдись копии — тег перестал бы отличать наше слово от чужого
    /// одинаково у двух носителей, и `Distress::Diverged` заговорил бы на ровном месте.
