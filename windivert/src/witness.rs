...
/// не его код: реальный `Act` живёт в крейте `reflex`, от которого этот крейт нарочно не зависит
/// (докблок крейта, раздел «предел IntoCarrier»; там же — почему). Воспроизведена ЗДЕСЬ ровно
/// затем, чтобы над типом БЕЗ `CanAsk` эта функция не собралась — тем же способом, каким не
/// собрался бы `Act::<T>::ask` над таким же типом.
///
