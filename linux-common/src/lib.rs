//! ОБЩИЙ КОНТРАКТ ЯДРА И USERSPACE ДЛЯ КРЫЛА `tc` — И ОНО ПОМЕЧЕНО УСТАРЕВШИМ (05.09.2026).
//!
//! Крейт держит то, о чём обязаны договориться eBPF-программа и её загрузчик: действия над
//! потоком, статистика стира, диапазон портов анти-петли. Оба его потребителя
//! принадлежат закрытой ветке — крылу `tc`.
//!
//! Атрибута здесь нет намеренно: крейт собирается под `no_std` для цели BPF, и пометка каждого
//! имени зашумила бы сборку ядра, ничего не добавив к сказанному в `reflex-linux/src/lib.rs`.
...
/// Судьба кадра на шве прозрачного L2-перехвата.
///
...
/// Режим лифта — ЧТО вообще входит в датаплейн. Троичный, а не булев: прежний `STEER_ALL`
...
/// значение по умолчанию». Откат в `Surgical` при пустых `STEER_TARGETS` означал бы, что машина
...
pub enum SteerStat {
    ...
    Rewritten = 3,
    /// ЗРЕНИЕ: транзитный (НЕлифтнутый) кадр, чей payload начинается как TLS-рукопожатие —
...
/// Анти-петля ловца по зарезервированному src-порту (`portmark`). Direct-проба ловца уходит вниз
/// через роутер-потребитель (default route); тот NAT'ит её
/// и заворачивает обратно в мост (хэйрпин) → она RE-входит в `reflex_steer` на клиент-ingress. Без метки
/// steer лифтит её заново (dst ∈ STEER_TARGETS) → self-loop(шторм, риск брика). МЕТКА: потребитель биндит
...
/// userspace-узнавание порта пробы (симметрия контракта eBPF↔userspace; потребитель биндит в диапазон).
...
/// контракт eBPF (гейт лифта) ↔ userspace (потребитель биндит src-порт из диапазона `PROBE_PORT_LO..=HI`).
...
/// `reflex_observe` в `RingBuf`, потребляется userspace-witness'ом (`inline_witness` у потребителя).
...
///
/// БЫЛО 512 И ЭТО ОКАЗАЛОСЬ ДОПУЩЕНИЕМ, А НЕ ЗАМЕРОМ (13.08.2026, машина владельца): исходный
...
///
/// ПЯТЫЙ БАЙТ ДОБАВЛЕН ПО ЗАМЕРУ В ПОЛЕ (13.08.2026, машина владельца, 0.3.5): без него гейт
...
/// Guarded-решение перехвата: снимаем кадр
/// с транзита ЛИШЬ при готовой несущей; не-цель никогда не снимаем (Surgical); не готова —
/// fail-open на транзит (закон потребителя). Чистая функция свежих наблюдений, без снапшота-веры.
...
mod tests {
    ...
    // Таблица истинности атома — 4 строки, исчерпывающе.
    ...
    /// карте не лифтит ничего, то есть машина тихо перестаёт открывать заблокированное и выглядит
    ...
    /// НАБЛЮДЕНИЕ АДРЕСУЕТ ПОТОК, А НЕ ЦЕЛЬ (срез 1).
    ///
