...
//! потоком, статистика стира, диапазон портов анти-петли. Оба его потребителя — `reflex-linux`
//! под фичей `tc` и `nevod/ebpf` — принадлежат первому неводу, который закрыт.
//!
