...
ПРЕДЕЛ, НАЙДЕННЫЙ ЕГО ЖЕ ЗЕЛЁНЫМ ПРОГОНОМ. Он читает подписи предметов, ОБЪЯВЛЕННЫХ в
`reflex/src/`. У чужого типа, который фасад лишь перепродал (`pub use reflex_engine::parse::Read`),
объявления здесь нет — rustdoc не кладёт его нутро в наш указатель, и ни груз варианта
(`Read::Tcp(Wire)`), ни поля перепроданной структуры (`Datagram.dst: Addr`) он не видит. Первый
зелёный прогон 18.09.2026 объявил дверь замкнутой, не посмотрев ни в один такой вариант; течь нашлась
руками через час, когда понадобилось назвать `Wire`. Чинится это разбором JSON соседних крейтов и
сшивкой указателей — работа не сделана, предел назван вслух, чтобы зелёный не читался шире, чем
значит.
    ...
    while growing:
        ...
        for item_id in list(found):
            ...
            if "struct" in inner:
                ...
                # ГРУЗ ВАРИАНТА — то же, что поле структуры: `Read::Tcp(Wire)` требует назвать
                # `Wire`. Без этой ветки сторож молчал о целом классе течей — первый его зелёный
                # прогон (18.09.2026) объявил дверь замкнутой, не посмотрев ни в один вариант.
                kind = inner["variant"]["kind"]
                if isinstance(kind, dict) and "tuple" in kind:
                    children += [f for f in kind["tuple"] if f]
                elif isinstance(kind, dict) and "struct" in kind:
                    children += kind["struct"]["fields"]
            elif "trait" in inner:
...
def main(path):
    ...
    print(
        "\nЛечится одним из двух: реэкспортом на фасаде (имя ОБЯЗАН назвать потребитель) либо\n"
        "закрытием подписи (не обязан). Третьего — «возьмёт вторую зависимость» — нет.\n"
        "Критерий обязанности: назвать надо, чтобы ОБЪЯВИТЬ (подпись, поле, свой тип) или ПОСТРОИТЬ\n"
        "значение. Разобрать по месту и прочитать поле можно без имени — такое не выставляется."
    )
    return 1
