...
//!
//! Закон по БАЙТАМ ЦЕЛИ сверх заголовков (`up.packets == 0` — разговора не было; рукопожатие есть, а
//! байтов сверх заголовков нет — тихий дроп; байты есть — жива), порог — по ВОЗРАСТУ потока: два
//! наблюдения умещаются в burst рукопожатия ДО ответа, и лишь возраст отличает «ещё не ответила» от
//! «уже не ответит» (см. докблок `edge_detect`). Тесты гоняют поток пакет за пакетом, перенося фазу через марку — `mark_after`
...
/// Клиент отдал запрос: три пакета, 400 байт (400 − 3×60 = 220 > 100). ЦЕЛЬ ПРИ ЭТОМ МОЛЧИТ: её
/// байты — ровно вес её же кадров (56 на кадр, типичный `ACK` с опциями), то есть сверх заголовков
/// ноль. Фикстура называет вес честно: поставь сюда прежнюю константу 60 на любое число кадров —
/// и край утверждал бы физически невозможное (два кадра общим весом в один).
fn with_request(up_pk: u64, age_s: u64) -> TestEdge {
    edge(3, up_pk, 400, up_pk * 56, age_s)
}

/// То же, но цель ОТДАЛА данные: сверх веса своих кадров ещё `payload` байт.
fn with_answer(up_pk: u64, payload: u64, age_s: u64) -> TestEdge {
    edge(3, up_pk, 400, up_pk * 56 + payload, age_s)
}
...
/// ЦЕЛЬ, КОТОРАЯ ТОЛЬКО ПОДТВЕРЖДАЕТ, ЖИВОЙ НЕ СЧИТАЕТСЯ. Прежде порогом живости был счёт пакетов
/// («≥2 — жива»), и на этой самой клетке дверь молчала обеими половинами: край отпускал разговор,
/// а проводная половина `NoBytes` на TCP не говорит вовсе. Подтверждение — пакет без данных, и
/// спрашивать о байтах надо байты.
#[test]
fn a_target_that_only_acknowledges_is_not_taken_for_alive() {
    let s = silence();
    let (mark, first) = mark_after(s, with_request(1, 0));
    assert!(said(&first).is_empty(), "первое — подозрение");
    // Цель прислала ещё два подтверждения (up_pk = 3), данных не дала, возраст за окном.
    let (mark, second) = mark_after(s, with_mark(with_request(3, 5), mark));
    assert!(
        said(&second).contains(&Distress::NoBytes),
        "подтверждения без данных — не ответ: {:?}",
        said(&second)
    );
    assert_eq!(
        mark,
        Memo::new(layout(), Phase::Confirmed, 3).apply_to(0),
        "сказанное закрепляется фазой, а не подозрением"
    );
}

/// Цель ОТДАЛА байты сверх заголовков — жива: `Released`, НЕ `Confirmed`, и возраст тут ни при чём.
/// Порог живости — данные, а не движение: марка обязана нести `Released`.
#[test]
fn a_target_that_gave_bytes_is_released() {
    let s = silence();
    let (mark, first) = mark_after(s, with_request(1, 0));
    assert!(said(&first).is_empty());
    // Даже при возрасте за окном отданные данные отпускают поток.
    let (mark, second) = mark_after(s, with_mark(with_answer(2, 300, 5), mark));
    assert!(said(&second).is_empty(), "цель ответила делом — жива");
    assert_eq!(
        mark,
        Memo::new(layout(), Phase::Released, 2).apply_to(0),
        "порог живости — байты сверх заголовков, поток отпущен"
    );
}
...
fn an_answered_flow_is_not_a_silent_drop() {
    let (_mark, verdict) = mark_after(silence(), with_answer(5, 1_200, 3));
...
/// СОСЕД, ПИШУЩИЙ ВНЕ НАШЕЙ МАСКИ, — НЕ ЧУЖОЙ ПИСАТЕЛЬ, А НЕТРОНУТАЯ МАРКА.
///
/// Оплачено стендом 20.09.2026 (невод 3 на живой цензуре): продукт метит укрытые цели маркой
/// маршрутизации в МЛАДШЕМ байте (`0xCC` — ею правило ядра выбирает ногу), а прибор края читает
/// свою раскладку `0x0FFF_E000`. Пересечения нет ни битом, но `read` отдавал `Foreign` со ВСЕМ
/// словом, а прибор судил по `theirs != 0` — и каждый пакет укрытой цели рождал `Diverged`.
/// Замер того прогона: 41 беда из 43 за четыре запроса, все после карантина, и цель не выходила
/// из него никогда.
///
/// Закон, который здесь стережётся: судить дозволено ТОЛЬКО по битам под своей маской. Вне её
/// сосед законен — это сказано с другой стороны в `foreign_bits_survive_the_write` (наша запись
/// его биты сохраняет), и читатель обязан говорить то же, что писатель.
#[test]
fn a_neighbour_writing_outside_our_mask_is_not_a_foreign_writer() {
    let outside = 0x0000_00CC;
    let (_mark, verdict) = mark_after(silence(), with_mark(with_request(1, 2), outside));
    assert!(
        !said(&verdict)
            .iter()
            .any(|distress| matches!(distress, Distress::Diverged { .. })),
        "марка вне нашей маски — не находка: {:?}",
        said(&verdict)
    );
}
