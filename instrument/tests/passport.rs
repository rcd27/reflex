...
//!
//! **Замер: прибор без паспорта уже заводился.** `UnreachedInstrument` (ответы цели не доходят)
//! нёс `Mealy` и паспорта не имел, и не заметило этого НИЧТО — ни сборка, ни 964 теста, ни аудит:
//! его скрипты читают паспорта, а прибор без паспорта для них просто не существует.
//!
