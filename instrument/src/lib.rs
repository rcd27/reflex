    ...
    (Self::Out, Self::Log): Spoken<Signals = smallvec::SmallVec<[<Self as Instrument>::Signal; 2]>>,
{
    ...
    /// Имя прибора в публичном круге. Им подписываются его показания ([`MealyExt::by`]).
...
/// Снять показание с одного наблюдения — для поверок и потребителей без потока. Спрашивается тем же
