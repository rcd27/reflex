...
// Каждый прибор — свой модуль, и все они ЗДЕСЬ, а не по месту потребления: детекция, расселённая
// по крейтам, даёт свой ответ на «что мы видим» в каждом, и ответы расходятся молча. Прибор
// возвращает своё ПОКАЗАНИЕ, не беду продукта: перевод в Distress/Sighting/Finding — работа
// потребителя.
pub mod agreement;
pub mod ask;
pub mod departure;
    ...
    (Self::Out, Self::Log): Spoken<Signals = smallvec::SmallVec<[<Self as Instrument>::Signal; 2]>>,
{
    ...
    /// Публичные имена событий — реестр, снятый с типа: `Debug` сменил бы метку молча.
