...
//!
//! # ПАРК ШИРЕ ФАСАДА, И ЭТО НЕ ДОЛГ
//!
//! Замер 19.09.2026: из семнадцати приборов парка пятеро наружу не выходят — `Trust`, `Agreement`,
//! `History`, `Pace`, `Sag`. Двери им не открыты НАМЕРЕННО: дверь без замеренного спроса есть
//! механизм, потреблённый ноль раз, — он стоит ноль и хуже того, выглядит готовым. В тот же день
//! это дважды стоило нам дефектов: `listened` жила четыре дня непрошенной и соврала бы первому
//! потребителю, а предпосылка местного носителя требовала того, чем он не пользуется.
//!
//! УСЛОВИЕ ОТКРЫТИЯ — замер, а не желание. Образец, поданный потребителем по `Trust` (19.09.2026,
//! пять подмен на семнадцати проверенных целях): «дверь нужна, когда замер покажет, что подмена
//! приходит на целях, которые НЕ под лечением и БЕЗ бед по доставке, — тогда это новая беда, у неё
//! свой исход, и слово наружу имеет адресата». До такого замера спрос не назван, а сказать, ЧТО
//! потребитель сделает со словом, нельзя — значит и слова не надо.
//!
//! Приборы при этом живут и проверяются законами своих модулей: закрытая дверь не есть мёртвый
//! код, она есть НЕОБЪЯВЛЕННАЯ способность.
