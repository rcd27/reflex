...
pub struct Point {
    ...
    /// без него значило бы принять смену выборки за смену инспекции.
    pub alive: usize,
    pub pierced: usize,
    pub clean: usize,
    pub content: usize,
...
/// абсолютное число упало бы вместе с ним, не сказав ничего об инспекции.
...
/// НИКАК, и ровно поэтому спор о дрейфе инспекции был спором о вере: у обеих сторон не было ряда.
...
impl crate::Instrument for HistoryInstrument {
    ...
    /// РЯД МЕЖДУ ПРОГОНАМИ — единственный такой темп в парке. Дрейф инспекции меняется в часах, и
...
mod history_passport_tests {
    ...
    /// случайности: дрейф инспекции меняется в часах, суточный прогон не медленнее предмета.
    #[test]
