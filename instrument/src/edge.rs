...
/// Что прочли из марки. Клеток ТРИ, и третья заведена ценой стенда: «под нашими битами пусто» и
/// «по нашим битам писал другой» — разные вещи, а прежде они делили одну клетку `Foreign`, и
/// различал их потребитель, глядя на `theirs != 0`. То есть судил по слову ЦЕЛИКОМ, включая биты
/// ВНЕ нашей маски, где сосед законен по построению (см. `foreign_bits_survive_the_write`: наша
/// запись его биты сохраняет). Невод 3 на живой цензуре 20.09.2026 метил укрытые цели маркой
/// маршрутизации `0xCC` в младшем байте — пересечения с нашей маской ноль, — и каждый пакет
/// укрытой цели рождал `Diverged`: 41 беда из 43 за четыре запроса.
///
/// Различение вынесено в ТИП, а не оставлено значению: значение потребитель волен прочесть
/// как угодно, а клетку `match` заставит разобрать.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Recall {
    /// Наш тег — дальше наша же память (фаза и оттиск).
    Ours(Memo),
    /// По нашим битам писал ДРУГОЙ: под маской непусто, а тег не наш. Слово отдаётся целиком —
    /// оно улика для человека, а не предмет суждения: судить уже нечего, клетка различена.
    Foreign { theirs: u32 },
    /// Под нашей маской пусто. Вне её слово может быть занято кем угодно — и это НЕ находка.
    Untouched,
}
...
impl Layout {
    ...
    /// Прочесть марку. Сперва НАШИ БИТЫ: под маской пусто — `Untouched`, и слово вне маски нас не
    /// касается (сосед на машине законен). Затем тег: не наш — `Foreign` (остальным полям веры
    /// нет, они чужие).
    pub fn read(&self, word: u32) -> Recall {
        let under = word & self.mask;
        if under == 0 {
            return Recall::Untouched;
        }
        let packed = under >> self.shift();
