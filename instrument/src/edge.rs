...
/// упаковать без маски нельзя. Писать умеет только сама памятка (`apply_to`) — второй раскладке
/// взяться неоткуда, и кодек (`under_bits`) остаётся один.
///
...
/// про соседа, строит [`Layout::new`] своими числами.
///
/// ЧАСТНЫЕ НАРОЧНО, наружу торчит один [`Layout::preset`]. Прежде эти два числа стояли ТРЕМЯ
/// копиями в трёх крейтах (`reflex/src/nfqueue.rs`, `windivert/src/carrier.rs`,
/// `examples/detect-silent-drop/src/edge_main.rs`), и копии молчаливо расходились бы: тег — то,
/// чем `EdgeSilence` отличает СВОЁ слово от чужого (`Distress::Diverged`), а маска — то, под чем
/// оба носителя пишут фазу. Разойдись они, два носителя перестали бы быть сравнимыми, и
/// дифференциальный стенд, ради которого раскладка и совпадает, сверял бы разное, оставаясь
/// зелёным. Один дом отменяет расхождение ПОСТРОЕНИЕМ, а не сверкой чисел тестом.
const PRESET_MASK: u32 = 0x0FFF_E000;
const PRESET_TAG: u8 = 0b101;

impl Layout {
    /// Раскладка по умолчанию — та, что берут все носители, не знающие про соседа по марке.
    ///
    /// Не `Default`: `Default::default()` читается «пустая», а эта раскладка не пустая, а
    /// ВЫБРАННАЯ — конкретные биты и конкретная подпись писателя, и выбор назван выше.
    pub fn preset() -> Layout {
        Layout::new(PRESET_MASK, PRESET_TAG).expect("умолчание: 15 бит, ненулевой тег")
    }
