...
/// Тишина по величинам края: цель не ответила (`up_packets == Some(0)`) или молчит дольше окна
/// (`idle >= after`). Фазу и оттиск читает/пишет через марку — состояния в юзерспейсе не держит.
