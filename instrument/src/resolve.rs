...
impl ResolutionInstrument {
    fn read(&self, message: &DnsMessage) -> Option<Resolved> {
        ...
        // УСЕЧЁННОЕ СООБЩЕНИЕ НЕ СУДИТСЯ ВОВСЕ: оно само объявило себя неполным.
        //
        // Страж, а не третий член конъюнкции ниже, потому что предмет у него ДРУГОЙ. Улики ниже
        // говорят о поведении отвечающего; `TC` говорит, можно ли вообще верить счётчикам секций.
        // У обрезанного ответа секцию срезали по дороге — прочесть её пустоту как «долг не
        // исполнен» значит выдать отсутствие свидетельства за свидетельство отсутствия. Ту же
        // границу алфавит уже держит между `Opaque` (не понял содержимое) и `Torn` (знаю о
        // пропуске): незнание и знание о нехватке — разные буквы. Род тот же, что у
        // `Opaque::Truncated` в алфавите входа: буква, ПРЯЧУЩАЯ наблюдение, а не сообщающая факт
        // о мире, — и обращаются с ней одинаково, молчанием.
        //
        // Терять тут нечего: клиент на `TC` переспрашивает по TCP, и тот ответ прибор рассудит.
        // Замер (`dig +norec +dnssec +bufsize=512 +ignore`): `a0.org.afilias-nst.info`
        // и `a.dns.ripn.net` на несуществующее имя — `aa tc`, AUTHORITY: 0, две зоны из двух.
        if message.truncated {
            return None;
        }

        // Вопрос не про адрес — не предмет: в ответе на `AAAA` записей `A` нет по построению,
        // прибор, отвечающий на это «адрес стёрт», выдаёт свою слепоту за факт о мире.
        ...
        //
        // И это не эвристика, а ПРЯМОЕ НАРУШЕНИЕ ОБЯЗАТЕЛЬСТВА: RFC 2308 §3 требует (MUST) от
        // авторитетного отказа нести `SOA` со сроком кеширования отрицания. Отвечающий сам
        // объявил себя тем, кого это MUST связывает, — и его нарушил. Улика, стало быть, не в
        // признаке, а в РАСХОЖДЕНИИ заявленной роли и исполненного долга, и от списка внутренних
        // суффиксов она не зависит вовсе. Побочно §5 («отрицание без `SOA` кешировать не следует»)
        // объясняет, почему клиент на такой отказ переспрашивает.
        //
        // Замер на линии (`dig @8.8.8.8`): подделка (`nnmclub.to`, `rutor.info`,
...
impl crate::Instrument for ResolutionInstrument {
    ...
    const LIES: &'static [&'static str] = &[
        "РАСХОЖДЕНИЕ РОЛИ И ДОЛГА — УЛИКА, А НЕ ДОКАЗАТЕЛЬСТВО. Обвиняет КОНЪЮНКЦИЯ: присвоенная \
         ...
         БЕЗ `SOA` и БЕЗ `tc` (2 из 2) — механизм не усечение, страж выше его не ловит, и такой \
         законный отказ прибор объявит подделкой. Список внутренних суффиксов по-прежнему неполон, \
         но цена его ниже: внутренний резолвер, исполняющий RFC 2308, не обвиняется вовсе.",
        "УСЕЧЕНИЕ — КНОПКА «МОЛЧАТЬ» В РУКАХ ПОДДЕЛКИ. Ответ с `TC` прибор не судит вовсе, и \
         инжект, поставивший этот бит, заставит его молчать. Цена уплачена сознательно: законный \
         усечённый отказ хозяина зоны выглядит ровно как подделка (`aa`, AUTHORITY: 0), и обвинять \
         его значило бы читать срезанную секцию как незаполненную. Кнопка эта не новая — одна \
         запись в AUTHORITY глушит прибор дешевле; и клиент на `TC` переспрашивает по TCP, так что \
         подделку придётся повторить там.",
        "ПОДДЕЛКА, ПРИЛОЖИВШАЯ `SOA`, НЕ ЛОВИТСЯ ВОВСЕ — и это цена конъюнкции, уплаченная за \
         молчание на честном хозяине зоны. На линии замера фильтр секцию не заполняет (замер выше), но \
         стоит это ОДНОЙ записи в инжекте. Улика умрёт молча: прибор просто перестанет говорить.",
        "СЧЁТЧИК — ЗАЯВКА ЗАГОЛОВКА, ТЕЛОМ НЕ ПРОВЕРЕННАЯ, И ТИПА ЗАПИСИ ОН НЕ ЗНАЕТ. Читается \
         `NSCOUNT`, а не содержимое секции: любая запись в AUTHORITY читается как исполненный долг, \
         а `NSCOUNT > 0` вовсе не значит «есть `SOA`» — отрицательный ответ вида TYPE 4 (RFC 2308 \
         §2.2) несёт в секции `NS` без `SOA`. Прибор устанавливает лишь одно: потрудился ли \
         отвечающий заполнить секцию ВООБЩЕ. Разбор секции сделал бы улику точнее ценой ещё одного \
         прохода по именам со сжатием.",
...
mod tests {
    ...
    /// секцию не заполняет.
    ///
    /// Замер на линии, `dig @8.8.8.8`: поддельный отказ (`nnmclub.to`, `rutor.info`,
    ...
    /// УСЕЧЁННОЕ СООБЩЕНИЕ НЕ СУДИТСЯ ВОВСЕ — оно само объявило себя неполным.
    ///
    /// Предмет тут другой, чем у двух улик выше. `aa` и пустая секция говорят о ПОВЕДЕНИИ
    /// отвечающего; `tc` говорит о том, целое ли сообщение вообще. Секцию у него срезали по
    /// дороге, а не не заполнили, — и прочесть её пустоту как «долг не исполнен» значит выдать
    /// отсутствие свидетельства за свидетельство отсутствия.
    ///
    /// Замер, `dig +norec +dnssec +bufsize=512 +ignore`: авторитетные `a0.org.afilias
    /// -nst.info` и `a.dns.ripn.net` на несуществующее имя — `aa tc`, AUTHORITY: 0, две зоны из
