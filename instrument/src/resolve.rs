...
//!
//! Формулировка тикета: «его резолв уводят на подставной резолвер, и мы об этом не знаем: DNS не наблюдается
...
impl ResolutionInstrument {
    fn read(&self, message: &DnsMessage, _now_ms: u64) -> Option<Resolved> {
        ...
        // `NXDOMAIN` на каждое имя. ЦЕНА: настоящий перехват отвечает тем же `NXDOMAIN`, но
...
impl crate::Instrument for ResolutionInstrument {
    ...
    /// прибор начнёт видеть перехват резолва, то есть ровно тот случай, ради которого он и нужен.
...
mod tests {
    ...
    ///
    /// ЦЕНА НАЗВАНА: настоящий перехват отвечает `NXDOMAIN` с флагом `aa` от неавторитетного
