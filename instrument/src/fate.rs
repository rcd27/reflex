//! СУДЬБА СОЕДИНЕНИЯ — типология, ОБЩАЯ для прибора и обоих движков.
//!
//! Модель здесь же: `zond/model/ByteOracleSearch.tla`. Невод 1 держит свою копию
//! (`nevod/src/ladder/fate.rs`, #284) и прямо говорит зачем: «копировать механизм не нужно;
...
impl crate::Instrument for ObservedInstrument {
    ...
    /// УРОВЕНЬ: судьба соединения решается тем, отозвалась ли цель; TLS закрыт, и выше этого уровня он пассивно не видит.
    ...
    /// КОНЕЦ ПЛЕЧА — то есть отложенный трафик. Для судьбы соединения это законно: судьба
    /// доопределяется, только когда разговор кончился.
    ...
    const LIES: &'static [&'static str] = &[
        ...
        "`Observed` ЖИВОЙ, А КРУГ МЁРТВЫЙ. Сам вариант читают `nevod2::flow` и `nevod2::session`, \
