...
//! терпением — константой). Замер: первый повтор `ClientHello` ~360 мс, наше терпение
...
pub struct RetransmitInstrument {
    ...
    down: u32,
    fired: bool,
    /// Носитель объявил потерю — `down` перестал быть свидетелем. Ноль внизу с этого мига значит
    /// «не знаю», а не «цель молчала», и прибор обязан молчать: §7, своя слепота не улика.
    blinded: bool,
}
    ...
    fn step(self, event: Self::In) -> (Self, Self::Out, ()) {
        ...
        let (state, signals) = match event {
            reflex_core::DetectorEvent::Packet { input, at } => match input {
                ...
                // Голова клиента — тоже просьба: `ClientHello` первым дропается на линии замера.
                ...
                // Ослепший прибор о повторе не судит: обвинение строится из ПАРЫ «просьба видна,
                // ответа нет», а дыра забрала вторую половину пары.
                Seen::Resent { .. } if self.blinded => (self, smallvec::SmallVec::new()),
                ...
                // ПОВТОР ЦЕЛИ снимает подозрение так же, как первый её ответ: предмет этого
                // прибора — «попросили, вниз ничего», а повторяющая цель отдала байты. Что её
                // повтор сам по себе есть улика ДРУГОГО рода (ответы не доходят), говорит своя
                // буква и скажет свой прибор; смешивать их в одном обвинении нельзя.
                Seen::Restated { count } => (
                    Self {
                        down: self.down.saturating_add(count),
                        ..self
                    },
                    smallvec::SmallVec::new(),
                ),
            ...
            // Замер: тот же мир, ответ 1400 байт, дыра вместо него — `Retransmit{480мс}`
...
impl crate::Instrument for RetransmitInstrument {
    ...
    const LIES: &'static [&'static str] = &[
        ...
        "ПОВТОР ЛЮБОГО СЕГМЕНТА, А НЕ ТОЛЬКО ПРИВЕТСТВИЯ. На нашей линии дропается `ClientHello` \
        ...
        "ПОТОК, ПОДХВАЧЕННЫЙ С СЕРЕДИНЫ ЛИБО РАЗОРВАННЫЙ ДЫРОЙ, ОСТАЁТСЯ НЕВИДИМЫМ. Просьбы прибор \
         не видел — либо носитель объявил потерю, и байты цели прошли мимо счёта; величины у \
         показания нет, и он молчит — это `Blind`, а не `Nothing`, и лечится оно другим прибором, \
         а не понижением порога здесь. После дыры прибор молчит до конца разговора: пропустить \
         беду дешевле, чем оборвать отвечающую цель.",
...
mod tests {
    ...
    fn a_tear_clears_the_accusation_against_a_target_that_answered() {
        ...
        assert!(
            said.is_empty(),
            "после дыры ноль внизу — собственная слепота, а не улика против мира (§7)"
        );
    }
