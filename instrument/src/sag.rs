...
mod tests {
    ...
    /// Подать ряд окон прибору и услышать сказанное.
    fn said(windows: &[u64]) -> Vec<Sag> {
        let (_instrument, out, ()) = SagInstrument.step(DetectorEvent::Packet {
    ...
    /// ПРЕДМЕТ ДОКБЛОКА, проверенный числом: ряд из оракула `sag(10mbit,2mbit,3)` — качалось хорошо,
    /// стало плохо и осталось плохим. Прибор обязан назвать ОКНО, планку и величину после.
    ///
    /// Ряд с маховиком в начале и хвостом в конце: оба неполны по природе (TCP разгоняется, файл
    /// кончается), и оба прибор отбрасывает — тест подаёт их нарочно, иначе проверялся бы не тот
    /// закон.
    ///
    /// ЧТО ЗНАЧИТ `at_window`, тест и закрепляет: это индекс в ИСХОДНОМ ряду, считая маховик,
    /// от нуля. Здесь плохо стало на четвёртом окне подряд, и число — `3`. Догадка о том,
    /// нумеруется ли оно от отброшенного маховика или от первого зачтённого, стоила бы читателю
    /// одного окна ошибки в отчёте; теперь её держит прогон, а не чтение кода.
    #[test]
    fn a_channel_that_went_bad_names_the_window_where_it_did() {
        let said = said(&[200_000, 1_250_000, 1_250_000, 250_000, 250_000, 90_000]);

        assert_eq!(
            said,
            vec![Sag {
                at_window: 3,
                before_bps: 1_250_000,
                after_bps: 250_000
            }],
            "просадка вдвое и глубже, держащаяся два окна, обязана быть названа"
        );
    }

    /// Вторая половина пары, и она названа ЛОЖЬЮ прибора в его же паспорте: канал, плохой С САМОГО
    /// НАЧАЛА, просадки не даёт — планка запомнила плохое. Прибор МОЛЧИТ, и молчание верно, хотя
    /// человеку медленно.
    #[test]
    fn a_channel_slow_from_the_start_is_not_a_sag() {
        assert!(said(&[90_000, 250_000, 250_000, 250_000, 250_000, 80_000]).is_empty());
    }

    /// Одиночный провал бывает от чего угодно — прибор ждёт ДВУХ окон подряд. Иначе всякая пачка
    /// давала бы тревогу.
    #[test]
    fn a_single_bad_window_is_not_a_sag() {
        assert!(said(&[200_000, 1_250_000, 250_000, 1_250_000, 1_250_000, 90_000]).is_empty());
    }

    /// Ряд короче четырёх окон — СЛЕПОТА, не пустота: маховик и хвост отброшены, судить не о чем.
    /// Лечится длинной загрузкой, и паспорт (`SILENCE = Blind`) обещает ровно это.
    #[test]
    fn a_series_too_short_says_nothing() {
        assert!(said(&[]).is_empty());
        assert!(said(&[1_250_000, 250_000, 250_000]).is_empty());
    }
}
