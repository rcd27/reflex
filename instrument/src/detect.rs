...
impl crate::Instrument for RstInstrument {
    ...
    const LIES: &'static [&'static str] = &[
        "НЕ РАЗЛИЧАЕТ ОТКАЗ СЕРВЕРА ОТ ИНЖЕКТА ПОСТОРОННЕГО (Н10). Оба приходят как \
        ...
        "ОДНО РАЗЛИЧЕНИЕ ВСЁ ЕЩЁ ЗАПИСАНО ДВАЖДЫ — но копий стало не три, а две. Копия в \
         `domain::wire::Rst` СНЯТА 02.09: она и была тем разъездом, который этот пункт \
    ...
    fn step(self, event: Self::In) -> (Self, Self::Out, Option<Measured>) {
        ...
        let (state, signals) = match event {
            reflex_core::DetectorEvent::Packet { input, at } => {
                ...
                // никогда (замер на живом трафике: цель за фильтром). Первую засечку повтор
...
impl crate::Instrument for SilenceInstrument {
    ...
    const LIES: &'static [&'static str] = &[
        ...
        "СЛОВО ОБ ИСТОРИИ (`NoBytes` — «не отвечала вовсе») ГОВОРИТСЯ О ТОМ ОТРЕЗКЕ, КОТОРЫЙ ВИДЕЛ \
         ПРИБОР, а видел он разговор с той буквы, на которой был заведён. Войди движок в живой \
         разговор с середины (перезапуск при открытых потоках) — и цель, ответившая до нашего \
         рождения, будет названа не отвечавшей. Оттого слово и отдаётся краю везде, где край \
         способен его сказать: счёт ядра ведётся с начала разговора, наш — с начала наблюдения. \
         Там, где свидетеля нет (датаграммы, край без часов, носитель без края), слово остаётся \
         здесь ВМЕСТЕ С ЭТОЙ ЦЕНОЙ: молчание о беде дороже, чем ошибка на потоке, пережившем \
         перезапуск.",
    ];
...
mod silence_tests {
    ...
    /// ретрансмиссией остался бы непойман (замер на живом трафике: цель за фильтром).
    #[test]
