...
//!
//! Слово у всех четверых — `Distress`: описание того, что видно, без слова о том, что мы
//! предпримем. Оно адресовано разговору, а не никому, — оттого и слово. Отсутствие беды сигналом
//! не является: прибор молчит.
...
///
/// ОТКРЫТ, ПОТОМУ ЧТО ВХОДИТ В ПОКАЗАНИЕ. Это наблюдаемое состояние прибора, а не его кухня: две
/// клетки из трёх глушат прибор навсегда, и расследователь, не видящий которая, не отличит «порог
/// не истёк» от «уже сказали» и от «разговор кончился».
...
///
/// Четыре оси, и все четыре несущие: сколько молчали, сколько байт отдала цель, ждёт ли человек
/// прямо сейчас и смотрит ли прибор ещё. Молчание есть беда, только если его КТО-ТО ЖДЁТ; но и
/// ждущее молчание беды не даёт, когда прибор уже пожаловался или разговор кончился, — а это
/// ветвление читает [`Watch`], и без него показание объяснить решения не может.
///
/// Показание, где `since_ms` за порогом, `bytes` в нуле и `awaiting` поднят, а слово пусто,
/// разбирается по четвёртой оси на ДВЕ клетки, а не три: `Open` здесь недостижим — решающая
/// ветвь тика сравнивает те же `since_ms`/`bytes`/`awaiting`, что вошли в показание, и при
/// `Open` она сработала бы сама, дав слово вместо пустоты. Остаются `Fired` — «уже сказали,
/// второй раз о том же не жалуемся» — и `Ended` — «цель попрощалась, и её молчание уликой не
/// является».
///
/// ПОРОГА ЗДЕСЬ НЕТ, И ЭТО НЕ ПРОПУСК: `after` есть настройка прибора, снятая при постройке и
/// одна на весь его век, а показание несёт то, что прибор НАБЛЮДАЛ. Кому нужен порог, тот берёт
/// его у экземпляра, а не у ленты наблюдений.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Measured {
    pub since_ms: u32,
    pub bytes: u32,
    pub awaiting: bool,
    /// СМОТРИТ ЛИ ПРИБОР ЕЩЁ. Ось решающая: `Fired` и `Ended` глушат его навсегда.
    pub watch: Watch,
}
...
mod rst_tests {
    ...
    /// ПОДПИСЬ ПЕРЕХВАТА — сброс от цели ДО того, как она отдала хоть байт.
    #[test]
