...
//! наблюдения, — и оставила МЕХАНИЗМ в `domain::wire`: поля `fired`, `answered`, `bytes`,
//! `watch`. Одно понятие получило две реализации, и они разъехались в тот же день:
//! `domain::wire::Rst` считал бедой только сброс ДО первого отданного байта, здешний прибор —
...
//! СНАРУЖИ комбинаторами `lmap`/`contextual` — см. `domain::wire::dressed`.
//!
