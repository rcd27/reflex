...
//! наблюдения, — и оставила МЕХАНИЗМ в `nevod2::wire`: поля `fired`, `answered`, `bytes`,
//! `watch`. Одно понятие получило две реализации, и они разъехались в тот же день:
//! `nevod2::wire::Rst` считал бедой только сброс ДО первого отданного байта, здешний прибор —
//! любой. По живой очереди работал первый, фикстурами поверялся второй.
//!
//! Теперь прибор есть `Detector`: `(State, Event) → (State, Signals)`. Состояние стоит в подписи,
//! часы приходят `DetectorEvent::Tick`, а доменное знание (чьё соединение, какая нога) надевается
//! СНАРУЖИ комбинаторами `lmap`/`contextual` — см. `nevod2::wire::dressed`.
//!
...
impl crate::Instrument for RstInstrument {
    ...
    const LIES: &'static [&'static str] = &[
        ...
        "ОДНО РАЗЛИЧЕНИЕ ВСЁ ЕЩЁ ЗАПИСАНО ДВАЖДЫ — но копий стало не три, а две. Копия в \
         `nevod2::wire::Rst` СНЯТА 02.09 (#320): она и была тем разъездом, который этот пункт \
         ...
         `nevod2_runtime::distress::trouble` над наблюдением ПЛОСКОСТИ: у него другой вход (`Sighting`, \
