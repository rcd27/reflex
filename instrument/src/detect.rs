...
/// разбирается однозначно ровно по четвёртой оси: `Open` значит «порог считается от `last`, и
/// он ещё не истёк», `Fired` — «уже сказали, второй раз о том же не жалуемся», `Ended` — «цель
/// попрощалась, и её молчание уликой не является».
///
