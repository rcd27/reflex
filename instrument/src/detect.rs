    ...
    fn step(self, event: Self::In) -> (Self, Self::Out, Option<Measured>) {
        ...
        let (state, signals) = match event {
            reflex_core::DetectorEvent::Packet { input, at } => {
                ...
                // никогда (замер на живом трафике: `rutracker.org` за ТСПУ). Первую засечку повтор
...
mod silence_tests {
    ...
    /// ретрансмиссией остался бы непойман (замер на живом трафике: `rutracker.org` за ТСПУ).
    #[test]
