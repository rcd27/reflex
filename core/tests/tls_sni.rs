...
//! TLS: ИМЯ ЦЕЛИ И ГРАНИЦЫ ЗАПИСИ — шесть законов вместо двадцати двух случаев.
//!
//! Не представители, выбранные рукой (`extract_sni_too_short`, `extract_sni_empty`,
//! `parse_too_short_data`, `parse_length_exceeds_data` — половина из двадцати двух): подборка
//! «подозрительных» входов ловит ровно то, что подозревали, а злой вход тем и зол, что его никто
//! не заподозрил.
//!
//! Вместо них то же, чем держатся опции TCP: ТОТАЛЬНОСТЬ перебором и НЕ ВЫДУМЫВАЕТ, плюс законы
...
//! подложенная в `session_id`, разбор не обманет. Замер: в каноне об этом НЕТ НИ СЛОВА
...
/// говорит: безымянная пара `(usize, usize)` обе половины называет одинаково. Первый читатель
/// прочёл вторую как конец и получил `slice index starts at 61 but ends at 13`. Повезло: срез
/// запаниковал. Будь длина больше смещения — вышел бы молча неверный кусок.
///
/// Проверяется срезом: `&hello[off..off + len] == name`. Без этого `span` был бы украшением —
/// число, о котором никто не спросил.
///
/// ЗАМЕР: `.span` не читается НИГДЕ в дереве, кроме этого теста. Поле не мёртвое, а ждущее: его
