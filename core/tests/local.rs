...
/// `SYN+ACK` ТОЖЕ несёт флаг `SYN` — не мелочь протокола, а причина, по которой наивный счётчик
/// «сколько раз видели SYN» считает ОДИН поток ДВАЖДЫ (клиентский `SYN` да ответный `SYN+ACK`).
/// `Mealy for Counts::step` (`core/src/local.rs`) от этой ловушки защищён ГЕЙТОМ
/// `self.opened.is_none()` — он взводит `opened` РОВНО РАЗ, кем бы из двух ни пришёл флаг — но
/// самого гейта тест не подменяет: он показывает голую причину, ради которой гейт нужен.
///
/// Живое свидетельство, не теория: задача 11 заводила количественный мутант («местный край теряет
/// `opened` у каждого второго ПОТОКА» — счётчик потоков вместо кадров, `lab/README.md`, раздел «Два
/// мутанта»), и первая редакция гейта считала СОБЫТИЯ флага `SYN` без разбора направления — каждый
/// поток получал ВТОРОЙ шанс попасть на чётность счётчика (клиентский `SYN`, потом ответный
/// `SYN+ACK`), и мутация самоизлечивалась на каждом потоке молча: живой прогон дал бы НЕПОНЯТНЫЙ
/// зелёный вместо явной красноты. Этот тест поймал причину ДО живого прогона; следующий, кто
/// заведёт свой счётчик по флагу `SYN`, увидит его здесь и не потратит прогон на те же грабли.
///
/// Это приемлемо, потому что гейт слеп к направлению НЕ случайно, а потому что цена слепоты мала:
/// `SYN+ACK` приходит сразу следом за клиентским `SYN` (доли RTT между ними), и `opened`, взведённый
/// ответом вместо запроса, занижает возраст потока на те же доли RTT — не на секунды, где живёт
...
fn syn_ack_тоже_несёт_флаг_syn() {
    ...
    let mut local = Local::new(Paper::new());
    local.saw_up(a_synack());
    assert!(
        local.edge_of(flow()).unwrap().age().is_some(),
        "SYN+ACK один, без клиентского SYN, тоже несёт флаг SYN и взводит opened"
    );
}
