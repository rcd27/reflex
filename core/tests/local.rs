...
/// ГЛАВНЫЙ ТЕСТ ПОДМЕНЫ: `Local::serve` обязан подать решению СВОЙ край, а не тот,
...
///
/// Живое свидетельство, не теория: количественный мутант «местный край теряет `opened` у каждого
/// второго ПОТОКА» (счётчик потоков вместо кадров, `lab/README.md`, раздел «Два мутанта») при
/// гейте, считающем СОБЫТИЯ флага `SYN` без разбора направления, САМОИЗЛЕЧИВАЕТСЯ молча: каждый
/// поток получает второй шанс попасть на чётность счётчика (клиентский `SYN`, потом ответный
/// `SYN+ACK`), и живой прогон даёт НЕПОНЯТНЫЙ зелёный вместо явной красноты. Кто заведёт свой
/// счётчик по флагу `SYN`, увидит причину здесь и не потратит на неё живой прогон.
///
