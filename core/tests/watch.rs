...
//!
//! Законы эти не про метки `nft` и не про программу десинка — те остаются у продукта. Здесь то,
//! что верно для всякого движка, перехватывающего трафик в userspace: цена — в дороге, а не в
//! вердикте, и потому разговор, который больше нечему научить, платить её не обязан.
...
/// рукопожатия, было бы совпадением, а не свойством. Две машины с разными порогами получили бы
