...
//!
//! Две болезни видны в одном выражении: вердикт ВЫЗОВОМ
//! (`msg.set_verdict(…); let _ = queue.verdict(msg)`), а `let _ =` выбрасывает единственный факт о
//! ДОСТАВКЕ. Четыре вердикта — четыре выброшенных свидетельства.
//!
//! Третья — свидетельство, несущее КОПИЮ полезной нагрузки: дёшево на вид и неограниченно по
//! памяти, при том что сырьё снимает независимый прибор, а дело помнит УЛИКУ, а не байты.
...
/// Носитель права ответа: на живом носителе — сообщение ядра, здесь — конверт с байтами.
struct Envelope {
    id: u32,
    bytes: Vec<u8>,
}
...
///
/// `Vec<u8>` в самом деле — аллокация на КАЖДЫЙ пакет и копия полезной нагрузки в записи. Байты
/// принадлежат носителю, пока он жив; дальше их место в сырье, снятом независимым прибором.
#[test]
