...
//!
//! Закон один на все часы, потому что порознь он расходится: записанный ЧЕТЫРЕ раза — в часах
//! записи (`pcap::on_grid`), в тестовых, в системных синхронных и в асинхронных — он у троих
//! считает одинаково, а у четвёртых (`tokio::interval`) дрейфует. Значит детектор, поверенный на
//! тестовых часах, под нагрузкой исполняется по другой сетке.
...
///
/// `u64::MAX` тиков на нулевой шаг выглядит как «очень много», а значит «мы не знаем»: сетки с
/// нулевым шагом не существует, и выдавать по ней узлы — выдумывать моменты, которых не было.
#[test]
...
///
/// Счёт вида `every * (nth as u32)` на миллисекундной сетке кончается через 49 суток аптайма, и
/// сетка прыгает в прошлое — то есть все временны́е операторы разом получают время, идущее назад.
/// У коробки, живущей месяцами, это не гипотеза.
#[test]
