...
/// СОСТОЯНИЕ КЛЮЧА, ПО КОТОРОМУ ДАВНО НИЧЕГО НЕ БЫЛО, СНИМАЕТСЯ (#294).
///
/// # Зачем
///
...
///
/// Прежде оператор об этом ПРЕДУПРЕЖДАЛ в документации — и первый же продуктовый вызов условие
/// нарушил. Предупреждение читают, когда пишут оператор, а не когда его применяют.
#[tokio::test]
