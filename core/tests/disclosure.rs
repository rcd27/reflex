...
//!
//! Предмет здесь не имя цели в `ClientHello`, а форма ВСЯКОГО сведения, добываемого из разговора:
//! имя, версия протокола, выбранный шифр, идентификатор соединения. Общее в них — не предмет, а
//! ПОРЯДОК: сведения только растут.
...
/// недетерминирован — две машины на одном трафике решат по-разному.
#[test]
...
///
/// ⊤ («противоречие») намеренно не заведён: частота явления не замерена, а тип под неизмеренное
/// потом не снимается. Но молчаливый выбор победителя — не отсутствие решения, а незаписанное
/// решение. Оттого правило приходит аргументом: не назвав его, слить два разных значения нельзя.
#[test]
