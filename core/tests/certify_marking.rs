...
//! «решение принято».
//!
//! Прозой у `Answer::Marked` это знание не держится: изнутри процесса «метку поставили» и «метку
//! прочитали» неотличимы совершенно — вердикт вынесен, `Delivered` подтверждает, счётчик движется,
//! а тот, для кого метка ставилась, о ней не узнал.
