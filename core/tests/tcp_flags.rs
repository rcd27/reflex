...
//!
//! Не ТАБЛИЦА по строке на тест (`display_syn`, `display_ack`, `display_fin`, …) — такая таблица
//! НЕ ПОЛНА и не умеет этого заметить. Флагов шесть, а отдельных строк печати в ней выходит пять:
//! `PSH` своей не имеет, он покрыт только парой. Добавь седьмой флаг (в TCP есть `ECE` и `CWR`) —
//! не покраснеет ни один из тридцати двух случаев.
//!
//! Механики крейта `bitflags` (`from_bits_truncate`, `from_bits`, `|`, `&`, `is_empty`,
//! `bits`-туда-обратно) здесь нет вовсе, хотя восемь случаев ушло бы на неё: свой тест на чужой
//! код не ловит наших ошибок, зато краснеет от чужих правок.
//!
//! Вместо них ИСЧЕРПЫВАЮЩИЙ ПЕРЕБОР: флагов шесть, значений 64, и перебрать их все дешевле,
