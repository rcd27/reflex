...
//!
//! Механизм не знает о продукте ничего: он отвечает на вопрос «сколько байт прошло за последние N
//! корзин» и «быстрее ли один поток другого».
...
/// ЧИСЛО ДЛЯ ЧЕЛОВЕКА БЕРЁТСЯ ИЗ ДРОБИ ОДНИМ МЕСТОМ, и это место называет случай, которого числом
/// не выразить.
///
/// Прежде деление стояло на трёх путях сразу (`tempo`, прибор просадки, здесь), и каждый делил
...
fn the_pace_in_human_units_is_computed_in_exactly_one_place() {
    ...
    assert_eq!(
        ...
        .per_second(),
        None,
        "нулевое окно: темп бесконечен, числом не назвать"
    );
}
