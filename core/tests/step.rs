...
//!
//! Категория стадий (`category.rs`) построена над потоком: её морфизмы реализованы как
//! `self.inner.map(f)`, то есть живут в категории ПОТОКОВ, а стадия остаётся фантомным ярлыком.
//! Оттого второй носитель — «пакет вошёл, вердикт вышел» — в неё не влезает: места для носителя
//! в конструкции нет вовсе.
//!
...
fn composition_carries_the_state_of_both_links() {
    ...
    assert_eq!(second, (1, 4), "второе слово: номер 1 (нумератор помнит), рекорд 4");
}
