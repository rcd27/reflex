//! ШАГ — морфизм категории стадий на носителе ЗНАЧЕНИЯ.
//!
//! БАЗА — носитель значения: морфизм есть машина Мили. Поток получается из неё функтором, обратно
//! — нет, и это не пробел, а несущая стена (`step::MealyExt::over`).
//!
//! Построй ту же категорию над ПОТОКОМ — и морфизмы зададутся отображением потока, то есть будут
//! жить в категории ПОТОКОВ, а стадия останется фантомным ярлыком. Второй носитель («пакет вошёл,
//! вердикт вышел») в такую категорию не влезает вовсе: места для носителя в ней нет.
