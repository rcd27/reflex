//! ДВЕРИ СПОСОБНОСТЕЙ БЭКЕНДА: НАБЛЮДЕНИЕ И ДРОП (#295, срез 4).
//!
...
//!
//! Vision §3.2 обещал ПОРТИРУЕМОСТЬ: смена backend не требует изменения frontend. Проверялось это
//! буквально — ОДНА функция-цепочка исполнялась над ДВУМЯ разными бэкендами, и её текст при этом
//! не менялся. Тест `the_same_chain_runs_over_two_different_backends` предъявлял именно это —
//! обещание третьего vision §3.2 — и снесён вместе со стадийной категорией.
//!
