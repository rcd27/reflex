//! ДВЕРИ СПОСОБНОСТЕЙ БЭКЕНДА: НАБЛЮДЕНИЕ И ДРОП.
//!
...
//!
//! Проверяется буквально: дверь к наблюдениям открывается только заявленной `CanObserve` (держит
//! компилятор, не соглашение), и `CanDrop` — не пустой маркер, а команда, которая СТРОИТСЯ и
//! ДОХОДИТ до стока.
//!
//! ДВЕ ЗАЯВКИ ЗДЕСЬ НЕ ПРОВЕРЯЮТСЯ, и это названо, а не забыто.
//!
//! ЗАПРЕТ «ОПЕРАТОР ПОСЛЕ ИНЪЕКТОРА» держался типом, не реализующим `Stream`: операторы до него не
//! дотягивались. Предъявить его нечем там, где инъекция перестаёт быть морфизмом цепочки вовсе —
//! цепочка отдаёт СЛОВО, применяет его драйвер, и продолжать не за чем.
//!
//! ПОРТИРУЕМОСТЬ (канон §9: смена backend не требует изменения frontend) предъявлялась буквально —
//! ОДНА функция-цепочка над ДВУМЯ разными бэкендами, текст её при этом не меняется. Здесь такого
//! прогона нет: цепочка есть значение, бэкенды суть драйверы, и переносимость держится формой, а
//! не тестом — то есть НЕ ЗАМЕРЕНА.
...
///
/// Третий бэкенд заведён именно за этим: двух, несущих `Command = Vec<u8>`, для такого
/// свидетельства не хватает.
#[derive(Debug, Clone)]
enum Verdict {
    Send(Vec<u8>),
...
///
/// Пустой маркер такого теста не допускает вовсе: значения, которым дроп выражается, при нём не
/// существует. Здесь проверяется первая половина — что
...
///
/// Проверка одна, а не две в разных местах: две записи одной идеи расходятся молча.
///
/// БЕРЁТСЯ СУЩЕСТВУЮЩАЯ ФИКСТУРА, а не заводится своя: четвёртый памятный бэкенд рядом с тремя
/// такими же был бы ровно тем дублированием — только в тестах.
#[tokio::test]
