...
//!
//! `Detector::step(self, event) -> (Self, SmallVec<[Signal; 2]>)` — уже машина Мили, написанная в
//! этой репе (вторая форма — `FlowTable`, `core/src/flow_table.rs:22`, та же машина по ключу, но
//! через `&mut self`, а не по значению; `Reactor`, третья такая форма, снесена 06.09.2026 вместе
//! с диалектом). Заведи мы здесь `&mut self`, форм стало бы три вместо двух; сведение их в одну —
//! предмет отдельного среза, и оно возможно ровно потому, что форма одна.
