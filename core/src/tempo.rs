//! Темп потока — как события шли во времени, а не сколько их было. Сумма и длительность вместе не
//! отвечают на «когда стало плохо»: мегабайт за две минуты и мегабайт за две секунды с последующим
//! молчанием неразличимы по обеим величинам, а переживаются противоположно. Меряет: `first_at`
//! (момент первого события), `max_gap` (самая длинная пауза МЕЖДУ событиями), темп по окнам (min и
//! max — «всегда медленно» против «просело»). Чистый: ни IO, ни часов, момент аргументом — прибор
//! проверяется таблицей. Понятий «запрос»/«ответ» здесь нет (пауза при молчащем и при ждущем
//! потребителе различимы, лишь зная, кто чего просил — знание домена, надстраивается сверху).
...
impl Tempo {
    /// Прибор заводится в момент начала наблюдения. Пауза до первого события считается ОТСЮДА (иначе
    /// время до отклика неотличимо от нуля у потока, молчавшего с рождения). `window` — за сколько
    /// считается темп: меньше — шум пакетизации, больше — теряется уже почувствованная пауза.
    ...
    /// Время идёт, событий нет. Без этого пауза видна лишь ПОСЛЕ того, как поток ожил — ровно тогда,
    /// когда потребитель уже ушёл.
    ...
    /// Единицы, добытые ДО начала наблюдения. Поток уже отвечал — первого события потребитель
    /// дождался, приписывать ему ожидание с нуля нельзя.
    ...
    /// Закрыть ВСЕ окна, чей срок вышел, — не одно. Редакция, закрывавшая по одному, схлопывала
    /// сорокасекундную паузу в ОДНО окно с единицами до и после, показывая высокий темп там, где
    /// экран замер. Пустые окна паузы обязаны быть посчитаны: они и есть провал.
    ...
    /// Сколько ждали первого события. `None` — событий не было вовсе, другой факт, чем «ждали долго»
    /// (число стёрло бы разницу «медленно»/«никогда»).
...
mod tests {
    ...
    /// Длинная пауза закрывает ВСЕ просроченные окна, не одно: редакция по одному схлопывала паузу в
    /// единственное окно с единицами до и после, показывая высокий темп там, где поток стоял.
    #[test]
