...
pub mod builder;
// ФУНКТОР В КАТЕГОРИЮ БЭКЕНДОВ (срез 4): способность стала ОГРАНИЧЕНИЕМ, а не пометкой.
pub mod backend;
pub mod capability;
pub mod certify;
pub mod checksum;
...
pub mod stack;
pub mod tape;
// ЗАПИСАННЫЙ ПРОВОД — симуляционный источник (срез 0). Под фичей, потому что читать файлы
// нужно не всякому потребителю: на машине провод живой. Зависимостей нет — только `std`.
#[cfg(feature = "pcap")]
pub mod pcap;
pub mod role;
pub mod sight;
