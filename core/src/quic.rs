//! QUIC-голова — имя цели из зашифрованного `Initial`. Возможно потому, что ключ выводится из
//! идентификатора соединения (DCID), лежащего в том же пакете открытым текстом (RFC 9001 §5.2):
//! шифрование `Initial` защищает от порчи в пути, не прячет содержимое от наблюдателя (так и DPI
//! различает цели по SNI). Зачем нам: без имени QUIC-цель ключуется сетью `/24` и знание не
//! переносится между узлами CDN; с именем она попадает в тот же ключ, что и её TCP-двойник.
//! Опознание цели с провода живёт в одном месте (до 30.08 QUIC-сторона жила в краю и звала
//! TLS-сторону) — чистые функции «байты → `Option<имя>`», место им в субстрате. `ring` подключён
//! фичей `quic`. Чего нет, названо: только клиентский `Initial` (ни 0-RTT, ни `Retry`); версии TLS
//! 1.2/1.3 не различаются; ECH не поддержан — а он и есть то, ради чего SNI перестанет читаться.
...
/// ИДЕНТИФИКАТОР СОЕДИНЕНИЯ из клиентского `Initial` — открытым текстом, до всякой расшифровки.
///
/// `None` — не `Initial`, не та версия, не разобрался. Зачем наружу: по нему различается ПОВТОР
/// рукопожатия. Клиент, не получивший ответа, шлёт `Initial` заново с ТЕМ ЖЕ `DCID`, но новым
/// номером пакета — байты датаграммы при этом другие, и сравнение «тот же кадр» повтора не видит.
/// У TCP ту же работу делает граница по `seq`; у QUIC единственное, что видно наблюдателю и не
/// меняется между попытками, — это `DCID`.
///
/// Отдаётся СРЕЗ исходной датаграммы, а не копия: сравнивать его вызывающему, хранить — его дело.
pub fn initial_dcid(datagram: &[u8]) -> Option<&[u8]> {
    parse_header(datagram).map(|header| header.dcid)
}
...
/// Разбор длинного заголовка. Тотальный: нехватка байтов даёт `None`, не панику — датаграмма с
/// провода бывает любой.
...
fn initial_keys(dcid: &[u8]) -> Option<Keys> {
    ...
    // Секрет уже есть PRK, повторно извлекать нельзя: `extract(&client)` с пустой солью прогнал бы
    // готовый секрет через HKDF ещё раз и дал другие ключи — расшифровка молча не удавалась и
    // читалась как «имени нет» (ошибка крипты неотличима от законного исхода без живого пакета).
...
/// Собрать `CRYPTO`-фреймы в один поток. `ClientHello` разложен по фреймам и НЕ обязан идти по
/// порядку — клеим по сдвигам (склейка «как пришло» дала бы перемешанный hello, имя не достать).
...
/// склеенным потоком: `ClientHello` не обязан помещаться в пакет (Chrome разбивает на два
/// `Initial`), склеивать надо по всему потоку — иначе половина имени теряется.
...
/// Достать имя из голого рукопожатия. В TLS поверх TCP `ClientHello` завёрнут в ЗАПИСЬ, в QUIC
/// записи нет — рукопожатие лежит голым, а готовый разбор (`reflex_core::tls`) ждёт запись.
/// Оборачиваем в заголовок записи, а не пишем второй разбор: своя реализация была бы ВТОРОЙ правдой
/// о том, что такое `ClientHello`.
...
mod tests {
    ...
    /// Нехватка байтов — `None`, не паника: датаграмма с провода бывает любой.
    #[test]
    ...
    /// Заголовок разбирается до номера пакета. Байты с провода: длинный заголовок, версия 1, DCID
    /// восемь байт, пустой SCID, пустой токен, длина 1232.
    #[test]
    ...
    /// Чужая версия не разбирается: у черновых другая соль, подставить свою наугад — получать мусор
    /// и толковать как «имени нет». Байтов даём вдоволь: обрыв массива падал на нехватке ДО проверки
    /// версии, и тест зеленел со снятой проверкой (найдено обезоруживанием).
    #[test]
    ...
    /// Короткий заголовок — не `Initial`: защищённые данные, ключей к ним нет.
    #[test]
    fn a_short_header_is_not_an_initial() {
        assert!(parse_header(&[0x40, 0x00, 0x00, 0x00, 0x01]).is_none());
    }

    /// Куски клеятся по сдвигам, не в порядке прихода: `ClientHello` разложен по фреймам, они не
    /// обязаны идти подряд.
    #[test]
    fn pieces_are_glued_by_offset_not_by_arrival() {
        // Куски встык, но в обратном порядке: куски С ДЫРАМИ дали бы один результат при любой
        // сортировке, и тест зеленел со снятой (найдено обезоруживанием).
        let backwards = vec![(6u64, b"second".to_vec()), (0u64, b"first-".to_vec())];
        assert_eq!(assemble(backwards), b"first-second".to_vec());
    }

    /// Дыра останавливает склейку: неполный `ClientHello` лучше склеенного неверно (из первого имя
    /// не достанется, из второго достанется ЧУЖОЕ — цель опознана как другая).
    #[test]
    ...
    /// Голое рукопожатие оборачивается в запись: в TLS поверх TCP `ClientHello` в записи, в QUIC —
    /// нет; готовый разбор ждёт запись, пишем обёртку, а не второй разбор.
    #[test]
