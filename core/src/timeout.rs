...
//!
//! # ПОТРЕБИТЕЛЕЙ В ПРОДУКТЕ НЕТ, И ОПЕРАТОР ОСТАВЛЕН РЕШЕНИЕМ (20.09.2026)
//!
//! Инвентаризация замерила: этот оператор срока не зовёт ни один прибор, ни одна
//! цепочка, ни один пример — только собственные тесты. Владелец оставил его, назвав это
//! странным: предмет у оператора есть, а спроса на него продукт не предъявил.
//!
//! Запись стоит здесь, чтобы следующий читатель не счёл код мёртвым и не снёс его походя:
//! мёртвым он выглядит по грепу, но им не является — он не позван, а не не нужен.
...
/// ПОЧЕМУ ПРЕДМЕТ СНЯТ С НАБЛЮДЕНИЯ — три рубежа уборки (§12.6), закрытым алфавитом.
///
/// Рядом с [`Expiry`], а не вместо него, и два имени здесь не дубль. `Expiry` — СЛОВО о мире,
/// сказанное оператором срока: «замолчал» либо «мы перестали ждать». `Departure` — ЗАПИСЬ О
/// СНЯТИИ, которую делает хранилище, и у неё есть третья причина, невыразимая сроком: машина
/// объявила свой конец сама. Слить их значило бы разрешить `Deadline::of(Expiry::Ended)` — срок,
/// который никто не отмерял; totality обоих алфавитов дороже одного общего.
///
...
pub enum Departure {
    ...
    /// мёртвое.
    Ended,
    /// СРОК: предмет молчал дольше порога. Тоже факт о мире, но вывод, опоздавший на порог.
    Idle,
    /// ПОТОЛОК: место кончилось у НАС. Единственная причина, снимающая ЖИВОЕ, — и потому
    /// единственная, о которой обязан узнать потребитель: молчаливая, она неотличима от «разговора
    /// не было».
    Ceiling,
}

impl From<Expiry> for Departure {
    fn from(expiry: Expiry) -> Departure {
        match expiry {
            Expiry::Idle => Departure::Idle,
            Expiry::Ceiling => Departure::Ceiling,
        }
    }
}
