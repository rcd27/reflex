//! `Local<C>` — форма, которую получает носитель БЕЗ ЯДЕРНОГО ДОМА. Ровно то, что `conntrack` +
//! `ct_mark` дают очереди ядра бесплатно (§ спеки «Local — край и дом состояния в юзерспейсе»),
//! `Local` строит сам: край в КАДРАХ (`EdgeView` — считать нагрузку значило бы разойтись с
...
//! ОТБРАСЫВАЕТ. Так `Local<QueueSocket>` кормит приборы СВОИМ счётом, а не `CtEdge`, хотя
//! обёрнутый `QueueSocket` его и строит рядом. Заведи `Local` второй закон о том же предмете в
//! каждом безъядерном крейте — носители разошлись бы молча. Здесь закон один, носителей — сколько
//! угодно.
//!
//! КРАЙ ЖИВЁТ НА `Serves`, А НЕ НА СООБЩЕНИИ (`Edging`, `core::held`), потому что вопрос не ГДЕ он
//! вычисляется, а КТО вправе его ПОДМЕНИТЬ. На типе сообщения (`C::Carrier::Carrier`) декоратору
//! подменять нечем: `impl Edging` на ЧУЖОМ типе не написать (правило сирот), да и `Edging` даёт
//! РОВНО ОДИН `Edge` на тип. Обернуть сообщение своим типом `serve` тоже не даёт: `decide`
//! получает лишь ССЫЛКУ (`&Held<Self::Carrier>`), а `Held::new` требует ВЛАДЕНИЯ — обёртка стоила
//! бы либо лайфтайма на `Terminal::Carrier` (трейт заперт нарочно), либо копии байт на КАЖДЫЙ
//! пакет. `serve` же уже стоит МЕЖДУ вызывающим и декоратором, и подмена достаётся бесплатно, тем
//! же неделимым шагом «взять и ответить». `Edging` остаётся ВНУТРЕННИМ помощником носителя, у
//! которого край и правда живёт в сообщении (`QueueSocket::serve` строит `Self::Edge` через него).
//!
...
/// Логика ОДНА, входов два по причине заимствования, а не по недосмотру: тот же довод, по
/// которому очередь ядра — `Serves`, а не `Source` (`held.rs`).
fn write_answer(
    table: &mut FlowTable<Counts, Flow>,
    flow: Flow,
    answer: Answer,
    ...
    C::Carrier: Observed,
{
    ...
    /// внизу принят и отброшен (`_their_edge`), не переслан. Так `Local<QueueSocket>` отдаёт
    /// приборам СВОЙ счёт, а не `CtEdge`, хотя обёрнутый `QueueSocket` его и строит.
    type Edge = LocalEdge;

    /// Носитель `C` сам зовёт СВОЙ `Terminal::apply` изнутри `C::serve` (таково устройство
    /// `Serves`, `held.rs`: взять и ответить неделимы, второго `&mut` на носителя не
