...
//! простою, но приколочен к `TcpSegment`/5-tuple: это таблица ПОТОКОВ, а нужен набор
...
///
/// # Почему `#[must_use]`, а не строка «потребитель обязан заметить»
///
/// `set.mark(k, now);` без всякого разбора собирался бы молча, и потолок стал бы невидимым — та
/// самая беда, от которой тип заведён. Атрибут переводит требование из прозы в высказывание
/// компилятора: он даёт ПРЕДУПРЕЖДЕНИЕ, а не отказ (сильнее в Rust не выразить — заставить
/// потребить значение язык не умеет), но «компилятор сказал» читают, а прозу — не всегда.
///
/// ```compile_fail
