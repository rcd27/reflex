...
//!
//! ЧИСЛО ЗАМЕРА, А НЕ ЧИСЛО СЕГОДНЯ: замер оставлен как был — он датирован, и правка задним числом
//! стёрла бы то единственное, ради чего он записан: размер беды на момент, когда её взялись лечить.
//! Из шести диалектов снесены два: `Reactor` — в тот же день, вместе со своим диалектом;
//! `Detector` — 06.09.2026, задачей 3 плана `absorbing-the-detector`. Живых диалектов на сегодня
//! четыре: `step::Step::step`, `engine::step::step` (`engine/src/step.rs`), `Plane::feed`
//! (`engine-nfq/src/plane.rs`), `NfqHandler::handle` (`engine-nfq/src/bin/plane-queue.rs`).
//!
...
//!
//! Довод про несложимость blanket-impl'ов был верен, пока в `core` жил ЕЩЁ ОДИН диалект машины
//! Мили рядом со `Step` — `Detector`. После его сноса (задача 3, 06.09.2026) конкурировать
//! блэнкету не с чем: диалект один, и вопрос когерентности снят вместе с диалектом, а не решён
//! обёрткой над ним.
//!
//! Обёртка `Detecting` снята следом за `Reacting`: она существовала не ради подъёма, а ради того,
