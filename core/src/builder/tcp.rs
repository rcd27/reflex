...
/// Максимум полезной нагрузки в ОДНОМ инжектируемом сегменте.
///
/// 1200 байт — не MSS пути, а заведомо безопасный пол: столько влезает при любом разумном MTU,
/// включая туннели с их заголовками (тем же числом пользуется QUIC как минимальным размером
/// датаграммы). Точный MSS пути инжектору неизвестен: он видит поток снаружи и ничьих SYN не
/// согласовывал, а ошибка в БОЛЬШУЮ сторону не деградирует, а УНИЧТОЖАЕТ — пакет не уезжает вовсе.
pub const SAFE_SEGMENT_PAYLOAD: usize = 1200;

/// Разложить непрерывный кусок потока в TCP-сегменты, ни один из которых не превышает потолка.
///
/// ЗАЧЕМ ЭТО ЗДЕСЬ, А НЕ У ПОТРЕБИТЕЛЯ. Инжектор кладёт байты на провод в обход собственного
/// стека, и потому теряет то, что стек делал за него: нарезку по MSS. Пакет длиннее пути не
/// фрагментируется и не отвергается с ошибкой — он просто НЕ УЕЗЖАЕТ, и потребитель, уже
/// дропнувший оригинал, тихо теряет байты. Замер 25.08: собранная TLS-запись 2770 Б, эмитнуто
/// два сегмента, на проводе один — 1388 Б.
///
/// Нарезка байт-в-байт прозрачна для получателя: TCP есть поток, и деление на сегменты его
/// содержимого не меняет. `seq` каждого куска считается от `seq` первого байта.
pub fn tcp_segments_capped(
    flow: &Flow,
    seq: u32,
    ack: u32,
    payload: &[u8],
    ...
    (0..payload.len())
        .step_by(cap.max(1))
        .map(|start| {
            let end = (start + cap.max(1)).min(payload.len());
            TcpBuilder::new()
                .flow(flow)
                .seq(seq.wrapping_add(start as u32))
                .ack(ack)
