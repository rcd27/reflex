...
/// Максимум полезной нагрузки в одном инжектируемом сегменте. 1200 байт — не MSS пути, а безопасный
/// пол (влезает при любом разумном MTU, включая туннели; тем же числом пользуется QUIC). Точный MSS
/// инжектору неизвестен (он видит поток снаружи, SYN не согласовывал), а ошибка в БОЛЬШУЮ сторону не
/// деградирует, а УНИЧТОЖАЕТ — пакет не уезжает вовсе.
...
///
/// Здесь, а не у потребителя: инжектор кладёт байты в обход своего стека и теряет нарезку по MSS —
/// пакет длиннее пути не фрагментируется, а просто НЕ УЕЗЖАЕТ, и потребитель, дропнувший оригинал,
/// тихо теряет байты (замер: TLS-запись 2770 Б, эмитнуто два сегмента, на проводе один — 1388 Б).
/// Нарезка байт-в-байт прозрачна: TCP есть поток. `seq` каждого куска считается от `seq` первого байта.
