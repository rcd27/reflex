...
//!
//!   КУДА ОНО ВЕРНЁТСЯ — НЕ РЕШЕНО, и это названо, а не спрятано за ссылкой. Спека алфавита §4
//!   вводит свидетеля движка и гарантирует способностью РОВНО ОДНО слово — `e.sever(…)` при
//!   `CanSever`; про инъекцию и про `CanInject` она не говорит ничего. Значит вернуть обещание —
//!   отдельный предмет, а не следствие уже написанного.
//!
