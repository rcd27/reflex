...
/// Переход вместо значения, НА КЛЮЧ: смена считается по ключу, а не по всему потоку — иначе две
/// цели, чередуясь, прошли бы насквозь.
///
...
///
/// Своего закона адресности у пары с моментом быть не может: общий закон пары накрывает всякую
/// пару, чьи половины суть слова одной области. Объяви момент своей областью — и два закона
/// пересекутся, а компилятор обвинит строку ЧАСТНОГО, то есть не ту правку, что заряд взвела:
/// падение выглядит беспричинным, а взводит его правка в чужом файле.
///
/// Имя вместо пары снимает вопрос целиком: у структуры свой закон, и общему закону пары она не
/// конкурент.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// в произведении, а слово едет соседу по стрелке и в общем потоке теряет отправителя.
///
