...
/// МАШИНА, УМЕЮЩАЯ СКАЗАТЬ «МОЙ ПРЕДМЕТ ИСЧЕРПАН». Канон §12.6 велит выводить уборку из входного
/// алфавита («TCP — по букве»), и §12.5 уточняет: из алфавита выводится ЧАСТЬ задачи. Спрашивают
/// здесь машину, а не букву, и вот почему обе оговорки нужны.
///
/// СПРОСИТЬ БУКВУ НЕДОСТАТОЧНО. «Пришёл `FIN`» — не «разговор кончился»: TCP закрывается двумя
/// прощаниями, и после первого поток жив в обратную сторону (half-close). Сложить два прощания
/// умеет только автомат, читающий буквы, — то есть машина.
///
/// СПРОСИТЬ БУКВУ ОПАСНО. `RST` выглядит концом, но для ЭТОГО продукта сброс — беда, а не норма
/// (`instrument::wire::Seen::Closed` разводит их прямо): поддельный `RST` от промежуточного
/// устройства и есть предмет наблюдения, стек клиента отвергает его по RFC 5961, и разговор
/// продолжается. Сними мы состояние по букве — потеряли бы его ровно там, где оно нужнее всего.
///
/// Умолчание `false` — «мой предмет сам не кончается», честный ответ целого класса машин (у
/// датаграмм прощания нет по существу). Такая область убирается сроком, как §12.6 и говорит.
///
/// ОБЪЯВЛЕНИЕ НЕ ЗАМЕНЯЕТ СРОКА, А ОСВОБОЖДАЕТ РАНЬШЕ НЕГО: прощание приходит не всегда — клиента
/// убили, NAT протух, связь оборвалась, и конца не скажет никто.
pub trait Ended {
    /// Предмет этой машины исчерпан: новых наблюдений по нему не будет.
    fn ended(&self) -> bool {
        false
    }
}
...
pub enum DetectorEvent<T> {
    ...
    ///
    /// `at` — момент ОБНАРУЖЕНИЯ, а не потери, и другого взять неоткуда: ядро выбросило сообщения
    /// раньше, чем мы позвали приём, а когда именно — не знает никто (`ENOBUFS` не несёт ни числа
    /// потерянных, ни времени). Отсюда цена, устранить которую нечем, но видеть обязано: между
    /// потерей и её объявлением наблюдения уже прошли КАК ПОЛНЫЕ, и прибор слепнет позже, чем
    /// перестал видеть. Всякое «окно, свободное от дыры» свободно лишь от ОБЪЯВЛЕННОЙ дыры.
    Torn { at: Instant },
}
...
impl<T> DetectorEvent<T> {
    ...
    /// Прячет ли буква наблюдения ЭТОГО разговора — то есть перестаёт ли отсутствие быть
    /// свидетельством. Закон один на дерево: приборы, судящие по отсутствию, зовут его, а не
    /// перечисляют буквы у себя — четыре копии одного критерия разошлись бы молча (§7, Д7).
    ///
    /// `Torn` — наблюдения были и не дошли. `Opaque` различается ПРИЧИНОЙ, и она уже записана в
    /// букве: `Truncated` есть ПОТЕРЯ (так названа в `parse::unread`) — кадр БЫЛ и мог нести ответ
    /// цели; `NotIpv4`/`NotOurProtocol` — законное и вечное свойство чужого трафика, ответом в
    /// нашем разговоре такой кадр быть не мог. Ослепнуть на нём значило бы онеметь зря.
    pub fn hides_observation(&self) -> bool {
        matches!(
            self,
            DetectorEvent::Torn { .. }
                | DetectorEvent::Opaque {
                    why: crate::parse::Unread::Truncated,
                    ..
                }
        )
    }
