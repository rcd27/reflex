...
///
/// Прежде дверь называлась `tick` и штамповала букву сама — нулевым номером узла, то есть сеткой,
/// которой нет вовсе. Оттого дыре доехать сюда было нечем, а обещание «дыра доходит до прибора»
/// оставалось неисполнимым при зелёной сборке.
