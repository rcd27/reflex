...
///
/// Через таблицу сегодня едут только `Packet` (в машину СВОЕГО ключа — [`FlowTable::process`]) и
/// `Tick` (во ВСЕ живые машины разом — [`FlowTable::tick`]): методы называют буквы поимённо, не
/// числом или перечислением алфавита, и не устареют молча от новой буквы. `Opaque` и `Torn` двери
/// сюда не имеют вовсе — у обоих нет ключа, но по разным причинам. У непонятого ключа нет ПОТОМУ
/// ЧТО разбор не добрался до адреса — вызывающему нечего подать в `process`. У дыры ключа нет
/// ПОТОМУ ЧТО носитель объявляет потерю на очередь целиком, не на пакет: мы не знаем, чьи
/// наблюдения пропали, и приписать её одному разговору значило бы соврать об остальных, которых
/// она не касалась. Заведись здесь дверь для дыры — она легла бы рядом с `tick` (одна на все живые
/// машины), не с `process` (ключа для неё нет и не будет).
