...
/// было, значит принять его показания за меру его точности): на живом ядре — разность счётчиков,
/// прибор иной природы.
pub trait Origin {
...
/// Дверь к наблюдениям — одна на весь крейт: закон входит той же дверью, что и движок
/// (`backend::observing`), иначе стенд измерял бы сам себя.
