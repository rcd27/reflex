...
pub trait ReflexExt: Stream + Sized {
    ...
    ///
    /// ПОЛИТИКА ОБЯЗАТЕЛЬНА (#295): `Lossy` со счётчиком либо `Backpressure`. Молчаливая потеря
    /// не входит в число вариантов — иначе всплеск источника тихо роняет копию в `sink`, и
    /// расследование недосчитывается целей, не зная об этом.
    ...
    ///
    /// Наружу выходит пара `(слово, показания)`, помеченная ключом, — и выходит она НА КАЖДЫЙ ШАГ,
    /// то есть на каждый пакет и на каждый тик, помноженный на число живых ключей. Молчание шага
    /// есть такой же его выход, как и речь: уронив пустое слово, подъём уронил бы вместе с ним
    /// показание того же шага.
    ///
    /// Значит цепочка, стоящая за подъёмом, едет на ПАКЕТНОМ темпе, а не на сигнальном, и то, что
    /// прежде видело лишь редкие срабатывания, теперь видит весь поток. Кому нужен прежний темп,
    /// тот ставит своё звено — уплощение слова или отсев молчания, — где это видно.
    ...
    ///
    /// Что будет до первого значения `other`, отвечает вызывающий: у оператора нет умолчания, и
    /// молчаливая потеря элементов до первого значения `other` не входит в число вариантов.
