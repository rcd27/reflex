...
pub struct DnsMessage {
    ...
    /// Объявило ли сообщение себя УСЕЧЁННЫМ (`TC`, бит 9 флагов) — то есть неполным.
    ///
    /// Предмет иной, чем у флагов выше: те говорят, ЧТО сказал отвечающий, этот — можно ли верить
    /// счётчикам секций вообще. У обрезанного ответа секцию срезали по дороге, и её пустота
    /// свидетельствует не о поведении отвечающего, а о размере датаграммы. Клиент на `TC`
    /// переспрашивает по TCP — вот тот ответ и судится.
    pub truncated: bool,
...
impl DnsMessage {
    pub fn parse(data: &[u8]) -> Option<Self> {
        ...
        Some(DnsMessage {
            ...
            // `TC` — бит 9 (считая от старшего): `QR·OPCODE(4)·AA·TC`.
