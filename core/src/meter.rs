...
impl Pace {
    /// ЕДИНСТВЕННОЕ МЕСТО, ГДЕ ТОЧНАЯ ДРОБЬ СХЛОПЫВАЕТСЯ В ЧИСЛО — для слова наружу, которое
    /// человек читает в единицах секунды. Внутри дерева темп сравнивается перекрёстным умножением
    /// ([`Ord for Pace`]) и не делится никогда: `slower_than` рядом объясняет, почему.
    ///
    /// `None` — окно нулевой длины: темп при нём бесконечен и числом не выражается. Прежде на трёх
    /// путях стояло `делим на max(1)`, и бесконечность превращалась в большое КОНЕЧНОЕ число,
    /// неотличимое от настоящего замера. Клетка честнее: назвать нечего — не называем (§7).
    ///
    /// ```
    ...
    pub fn per_second(&self) -> Option<u64> {
        match self.over_nanos {
            0 => None,
            over => u64::try_from(self.bytes as u128 * 1_000_000_000 / over as u128).ok(),
        }
    }
