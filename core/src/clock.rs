//! Часы — единственный источник времени и единственный источник тика. Закон один на запись
//! (`crate::pcap::with_ticks`) и на живой провод: замер нашёл СЕМНАДЦАТЬ мест вне тестов,
//! порождающих время самостоятельно, в пяти формах. Следствие у человека: где `Tick` вычисляется
//! внутри обработчика пакета, нет пакетов — время стоит, и всё, что заводилось ради тишины
//! (`Freshness::Silent`, `meter::expired`, устаревание, потеря цели), в тишине не срабатывает —
//! мёртвая нога умирает именно тогда, когда трафика нет. Часы здесь — трейт, системные живут в
//! `reflex-runtime` (ядро не знает рантайма). Ловушка виртуальных часов названа, потому что
//! оплачена: виртуальное время доказывает «сколько прошло», но не «оператор проснулся» — рядом с
...
pub struct TestClock {
    ...
    ///
    /// ЗАМКОМ, А НЕ АТОМИКОМ, и `Duration`, а не наносекундами. `AtomicU64` уносит ВЕСЬ КРЕЙТ на
    /// целях без 64-битных атомиков — `mips`, `mipsel`, `powerpc`: модуль публичный (рядом с
    /// [`OsClock`]), тестовый дубль компилируется в каждой релизной сборке и отнимает три
    /// архитектуры ради кода, который вне `#[cfg(test)]` не исполняется никогда. Замерено: `cargo check
    /// -p reflex-core --target powerpc-unknown-linux-gnu` → `cannot find AtomicU64 in atomic`.
    /// Замок взамен ничего не стоит: часы тестовые, спор за них идёт раз в продвижение, а счёт в
    /// наносекундах был следствием атомика («`Duration` не атомарен») и без него не нужен.
    ///
...
mod tests {
    ...
    /// время доказывает «сколько прошло», но не «нас разбудили» (оплачено потерянным waker'ом).
    #[test]
