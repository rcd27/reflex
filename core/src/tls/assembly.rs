...
///
/// Почти всё здесь велит, что с пакетом делать: пропустить, удержать, отдать собранную запись,
/// вернуть нетронутым. `Retransmitted` не велит ничего — он называет, ЧТО ЭТОТ ПАКЕТ ЕСТЬ: те же
...
///
/// Отложить нельзя ничего из сказанного здесь — провод держит байты до ответа.
