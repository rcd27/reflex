...
impl Mealy for RecordAssembler {
    ...
    fn step(self, event: Self::In) -> (Self, Self::Out, ()) {
        let (assembler, signals) = match event {
            ...
            //
            // Дыра — тем же путём: она объявлена носителем целиком и не говорит, что пропало именно
            // из ЭТОГО разговора. Досрочно бросать удержание по ней значило бы гадать; тик и так
            // оборвёт его по `hold_deadline`, если остаток не придёт вовсе — исход тот же, разница
            // только в задержке признания.
