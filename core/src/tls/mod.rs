...
//! ```
//!
//! Один предмет: до 30.08 QUIC-половина жила в краю и через границу подмодуля звала эту (собрав
//! `CRYPTO`-куски, заворачивала в TLS-запись и отдавала `extract_sni`) — разрезано по месту
//! написания, не по смыслу; теперь обе стороны здесь. В субстрате, а не в продукте: всё это чистые
//! функции «байты → `Option<имя>`», «узнать цель по проводу» есть general-purpose умение, как разбор
//! TCP (активный обход остаётся у `desync`). Чего нет, названо: версии TLS 1.2/1.3 не различаются;
//! ECH не поддержан — а он и есть то, ради чего SNI перестанет читаться.
...
pub enum TlsFragment {
    ...
    ServerHello,
    /// Тревога с её кодом. Код обязателен: `unknown_ca` (48) и `handshake_failure` (40) — разные
    /// беды с разным лечением («не доверяю сертификату» против «не договорились о параметрах»); без
    /// кода прибор сообщал бы «клиент чем-то недоволен», не сужая круг.
    Alert {
        level: u8,
        description: u8,
    },
...
/// неразделимо. `span` — `(offset, len)`, offset абсолютный. Связка убирает нелегальное состояние
/// «имя есть, позиции нет»: SNI либо целиком, либо нет.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Sni {
    pub name: String,
...
/// Предложен ли клиентом ECH (#317) — и, значит, может ли читаемое имя быть НЕ ТЕМ. Тип, не `bool`:
/// номер расширения меняется от черновика к черновику (`0xfe0d` — draft-13 и позже, `0xfe0e`/`0xfe0f`
/// — раньше), доля черновиков в поле есть прибор зрелости ECH; `bool` это выбросил бы, а ряд копится
/// с сегодняшнего дня.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Ech {
...
/// Признак ECH тем же структурным проходом, что и `sni`. Заведён до того, как понадобился: ECH — то,
/// ради чего SNI перестанет читаться, а иначе узнаем постфактум (продукт перестал лечить цели).
/// Стоит одного прохода по уже разбираемым расширениям и даёт РЯД: доля соединений с ECH во времени.
...
/// Чего не хватает читателю, чтобы увидеть ПЕРВУЮ TLS-запись целиком. `extract_sni` отвечает `None`
/// и на «это не TLS», и на «TLS не дочитан», а читателю нужно решить противоположное — ЖДАТЬ или
/// ИДТИ (слитый ответ стоил полю 815 флоу из 1500: в плечи уезжал обрезок, сервер ждал остаток).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum RecordNeed {
...
/// Сколько ещё нужно байт, чтобы первая TLS-запись стала полной. Судит ТОЛЬКО по объявленной длине.
/// Отдельная функция, не флаг у парсера: вопросы разные — парсер отвечает «что это»,
/// эта — «доколе читать».
