...
//! ```
//!
//! Один предмет, оттого обе стороны здесь: QUIC-половина, собрав `CRYPTO`-куски, заворачивает их в
//! TLS-запись и отдаёт `extract_sni` — разрежь их по крейтам, и разрез пройдёт по месту написания,
//! а не по смыслу. В субстрате, а не в продукте: всё это чистые
...
/// Предложен ли клиентом ECH — и, значит, может ли читаемое имя быть НЕ ТЕМ. Тип, не `bool`:
