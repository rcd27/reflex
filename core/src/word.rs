...
/// адресовано туда же, куда без него, и потому пара с моментом остаётся словом той же области, а
/// не требует, чтобы момент завёл себе адресата.
impl<S: Word> Word for (std::time::Instant, S) {
    type Of = S::Of;
}
