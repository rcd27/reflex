...
///
/// НОСИТЕЛИ БАУНДА НАЗВАНЫ, чтобы «всякий» был проверяемым словом, а не обещанием: операторов
/// ожидания в дереве два — [`crate::timeout::Timeout`] и [`crate::debounce::Debounce`], — и оба
/// требуют его в `impl Mealy`. До 20.09.2026 баунд не требовал никто: `may_wait` звали лишь два
/// теста, то есть закон предъявлялся сам себе. У каждого из двух рядом стоит `compile_fail` на
/// пакетном слове — сторож того, что баунд не вакуумен.
///
/// ```
