...
mod tests {
    ...
    ///
    /// Объявить область у момента можно ровно потому, что закон о паре здесь ОДИН. Стой рядом с
    /// ним частный закон для пары с моментом, эта строка дала бы пересечение реализаций — и
    /// компилятор винил бы строку частного закона, то есть не ту правку, что заряд взвела. Сборка
    /// этого модуля и есть свидетельство, что пересекаться не с чем: слово со штампом названо
    /// структурой ([`crate::detector::Stamped`]).
