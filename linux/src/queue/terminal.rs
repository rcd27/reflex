...
///
/// `Copy` не выводится: `Rewritten` несёт новые байты пакета, то есть ВЛАДЕНИЕ, и слово ответа
/// копируется не даром — притворяться обратным значило бы лгать о цене. Слово уходит по значению
/// один раз за пакет, `Clone` остался — на путях, где его и правда копируют.
...
pub enum Answer {
    ...
    /// (`NFQA_PAYLOAD` в том же сообщении вердикта). Предмет [`reflex_core::capability::CanRewrite`].
    ///
...
pub(crate) fn asked(answer: &Answer) -> Told<'_> {
    match answer {
        ...
        // НА УРЕЗАННОМ ЯДРЕ.
        //
        // Прямая запись `ct mark` в вердикте (`state`) требует `CONFIG_NETFILTER_NETLINK_GLUE_CT`.
        // Нет его — ядро атрибут ИГНОРИРУЕТ МОЛЧА: вердикт уходит, ошибки нет, память не легла. На
        // канарейке владельца (OpenWrt, 13.09.2026) рядом отсутствовал и
        // `nf_conntrack_timestamp` — признак той же урезанной сборки netfilter, — и в conntrack
        // все 119 разговоров стояли с `mark=0` при исправном всём остальном: беды рождались,
        // решения печатались, вердикты отправлялись.
        //
        // Марка ПАКЕТА этой опции не требует. Она разговора не переживает, и потому одна не
        // заменяет память — но потребителю достаточно перенести её правилом
...
impl QueueSocket {
    /// Роняло ли ЯДРО наши пакеты с прошлого взгляда — и, если да, момент обнаружения.
    ///
    /// Момент есть миг ОБНАРУЖЕНИЯ, а не потери: когда именно ядро уронило, не знает никто (счётчик
    /// не несёт времени), и выдумывать его значило бы соврать о наблюдении. Тот же закон, что у
    /// `Torn` от `ENOBUFS`, и он там записан прямо.
    ///
    /// Смотрим НЕ ЧАЩЕ раза в окно: чтение файла стоит открытия и разбора, а платить им за каждый
    /// кадр незачем — потеря заметна с задержкой в окно, и этого довольно. Цена задержки названа:
    /// беды, родившиеся между взглядами, слепотой не накроются.
    fn torn_by_kernel(&mut self) -> Option<Instant> {
        const LOOK: std::time::Duration = std::time::Duration::from_millis(200);
        let now = Instant::now();
        if now.saturating_duration_since(self.looked) < LOOK {
            return None;
        }
        self.looked = now;
        let seen = crate::queue::socket::user_dropped(self.queue)?;
        // Счётчик не читался прежде — сравнивать не с чем: запоминаем и молчим. Объявить дырой
        // первый же взгляд значило бы ослепить приборы на ровном месте.
        let before = self.dropped.replace(seen)?;
        match seen > before {
            true => Some(now),
            false => None,
        }
...
impl Serves for QueueSocket {
    /// Свой край — ровно тот, что строит `Edging` (`impl Edging for Held` выше): вычисляет его
    /// носитель, отдаёт наружу шов. Два места, одно значение.
    type Edge = CtEdge;
    ...
    {
        // СПРОСИТЬ ЯДРО, НЕ РОНЯЛИ ЛИ МЫ, — прежде чем отдавать наблюдения дальше.
        //
        // Переполнение приёмного буфера нашего сокета мы видим сами (`ENOBUFS` → дыра ниже). А
        // переполнение САМОЙ ОЧЕРЕДИ не видим никак: пакет не доехал даже до сокета. Ядро при этом
        // знает и считает — и до сих пор это знание до приборов не доезжало. Цена замерена в поле:
        // потерянный `SYN+ACK` даёт `Blackhole` по здоровой цели, потребитель уводит её в обход и
        // ломает то, что работало. Прибор прав по тому, что видел; видел он НАШУ потерю.
        //
        // Буква та же, что у `ENOBUFS`: дыра есть дыра, кто бы её ни проделал. Приборы на ней
        // слепнут законом, который уже есть (`DetectorEvent::hides_observation`), — новой работы
        // это не требует, требовалось лишь соединить ядерное знание с нашим алфавитом.
        if let Some(at) = self.torn_by_kernel() {
...
mod tests {
    ...
    ///
    /// ОБЕИМИ ДВЕРЬМИ, И ВТОРАЯ ПРОВЕРЯЕТСЯ НАРАВНЕ С ПЕРВОЙ: прямая запись `ct mark` требует
    /// `CONFIG_NETFILTER_NETLINK_GLUE_CT`, которого на боевой коробке нет, и там память ложится
    /// ТОЛЬКО через марку пакета с переносом правилом. Урони `skb_mark` — сборка промолчит, юниты
    /// останутся зелёными, а продукт на урезанном ядре перестанет лечить целиком.
    #[test]
    ...
    fn marking_reaches_the_verdict() {
        ...
        // ОБРАТНАЯ СТОРОНА БОЛЬШЕ НЕ СИММЕТРИЧНА, И АСИММЕТРИЯ ЗДЕСЬ — ЗАКОН, А НЕ НЕДОСМОТР.
        //
        // Метка в поле памяти была бы ОШИБКОЙ (выше): состояние, стёртое следующим пакетом, вместо
        // приказа маршрутизатору. А память в поле метки — законная ВТОРАЯ ДВЕРЬ: прямая запись
        // `ct mark` требует `CONFIG_NETFILTER_NETLINK_GLUE_CT`, которого на боевой коробке нет, и
...
/// Способность объявлена каноном §9.1 — что канон объявил, БОЕВОЙ носитель обязан держать:
/// способность, живущая только на небоевом пути, зелена в сборке и пуста в деле.
///
...
/// маршрутизации. Предмет `CanMark` (§9.1), и не тот же, что conntrack-марка: та переживает пакет,
/// эта — нет.
///
