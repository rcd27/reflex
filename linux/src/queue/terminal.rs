...
mod tests {
    ...
    /// `CONFIG_NETFILTER_NETLINK_GLUE_CT`, которого на урезанной сборке netfilter (OpenWrt
    /// канарейки) нет, и там память ложится
...
/// Способность объявлена каноном §9.1 — и держать её обязан тот носитель, которого открывает
/// цепочка потребителя: способность, живущая только у носителя из тестов, зелена в сборке и пуста
/// в деле.
///
