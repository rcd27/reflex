...
impl QueueSocket {
    ...
    pub fn open(queue: u16, base: TimeoutBase) -> Result<QueueSocket, QueueError> {
        ...
        // просим четвёртым сообщением. В зазор действует умолчание ядра — 1024 пакета, — и потому
        // окно это теряет пакеты лишь при всплеске в тысячу за время трёх netlink-сообщений.
        //
        // Названо потому, что окно искали: потребитель увидел четыре потери на старте и спросил,
        // не здесь ли они. Арифметика говорит, что не здесь (четыре против тысячи), и причина
        // нашлась другая — правило подачи стояло раньше, чем открылся сокет. Но зазор существует,
        // и знать о нём надо тому, кто однажды увидит потери на тысячах.
