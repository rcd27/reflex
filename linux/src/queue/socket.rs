...
/// СЛУШАЕТ ЛИ КТО-НИБУДЬ ОЧЕРЕДЬ — со слов ЯДРА, а не того, кто запускал слушателя.
///
/// # Чем оплачено (парк, 15.09.2026)
///
/// Сторонний процесс, которому отдана очередь, умер при старте на всех машинах (после сброса привилегий не прочёл свой lua), а
/// наблюдатель сутки докладывал об успехе: свидетельство выдавал `spawn`, то есть ОБЪЯВЛЯЮЩИЙ. Правило
/// очереди без слушателя тем временем ждало трафик, чтобы его уронить. Ядро же знало правду всё
/// это время: строка очереди в `/proc/net/netfilter/nfnetlink_queue` есть ровно тогда, когда к ней
/// привязан сокет.
///
/// `None` — файла нет (модуль не загружен или машина не та): клетка незнания, а не «не слушает».
/// Иначе на машине без модуля каждый слушатель объявлялся бы мёртвым, и «не смотрели» слилось бы
/// с «посмотрели — пусто».
pub fn listened(queue: u16) -> Option<bool> {
    let text = std::fs::read_to_string("/proc/net/netfilter/nfnetlink_queue").ok()?;
    Some(listened_in(&text, queue))
}

/// Разбор отдельно от чтения — по той же причине, что у [`dropped_in`].
fn listened_in(text: &str, queue: u16) -> bool {
    text.lines().any(|line| {
...
#[cfg(test)]
mod listened_tests {
    use super::listened_in;

    /// Строка настоящего вида: две привязанные очереди, поля в ядерном порядке.
    const REAL: &str = "  250  2434  0  2  65531  0  0  14843181  1\n  252 2909897129  0  2  65531  0  0  3344357  1";

    /// ПРЕДМЕТ: строка очереди в файле и есть привязанный слушатель — номер своей очереди
    /// отличается от чужой, а не от «чего-нибудь похожего».
    #[test]
    fn a_queue_line_is_a_listener() {
        assert!(listened_in(REAL, 250));
        assert!(listened_in(REAL, 252), "порт слушателя бывает любым числом");
    }

    /// Очереди в файле нет — её никто не слушает. Ровно это и было на машинах 15.09: 240 не было,
    /// а продукт говорил «несёт». Номер, совпадающий с чужим полем (`2434` — порт, `0` — счётчик),
    /// слушателем не считается.
    #[test]
    fn no_line_no_listener() {
        assert!(!listened_in(REAL, 240));
        assert!(!listened_in(REAL, 2434), "порт слушателя не номер очереди");
        assert!(!listened_in("", 250));
    }
}
...
impl QueueSocket {
    ...
    /// без него: прогон без этих переменных строк не видит. `REFLEX_LAB_TINY_QUEUE=N` сужает ёмкость очереди
    ...
    /// очередь). Оба нужны РАЗНЫМ оракулам: замер частоты `ENOBUFS` на неурезанной очереди (ёмкость
