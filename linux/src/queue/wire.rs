//! Сборка и разбор сообщений NFNL_SUBSYS_QUEUE — БЕЗ СОКЕТА: здесь чистые байты, а IO живёт у
//! `queue::QueueSocket`. Конфигурация очереди, вердикт с `NFQA_CT{CTA_MARK}` (чего крейт `nfq` не
//! умеет) и разбор пакета вместе с видом края.
//!
...
/// (Д8) требовал предъявить `queue_dropped` ненулевым, а очередь штатной ёмкости (1024) на
...
/// переполненном сокете (`ENOBUFS`) пакет роняет и считает в `queue_user_dropped`. Боевая машина 0.9.30:
