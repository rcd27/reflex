...
impl Capture {
    pub fn open(interface: &str, snaplen: usize, promisc: bool) -> Result<Self, String> {
        ...
        // FIXME: КОЛЬЦО ВМЕЩАЕТ 256 КАДРОВ, А ПОСЛЕ ПЕРЕПОЛНЕНИЯ ЗАХВАТ СЛЕПНЕТ НАВСЕГДА.
        //
        // Замер закона `CanObserve` на стенде сертификации (05.09.2026, `linux/tests/certify`):
        //
        //   послано 50 · 200          → увидено всё                       held
        //   послано 1000 · 50 000     → увидено РОВНО 256                 broken(Lost)
        //
        // Число выводится отсюда: при `snaplen 65535` размер кадра упирается в `block_size`, и
        // `(4096 / 4096) * 256` даёт 256 кадров на весь мегабайт памяти — кольцо тратит 1 МБ и
        // вмещает 256 пакетов, будь они хоть по 30 байт.
        //
        // ХУЖЕ ЧИСЛА ТО, ЧТО ЗАХВАТ НЕ ВОССТАНАВЛИВАЕТСЯ: 50 честных кадров, посланные через две
        // секунды ПОСЛЕ всплеска, не видны ни одного за оставшиеся шесть секунд окна. Контрольный
        // прогон с той же задержкой, но без всплеска, держится — значит виноват всплеск, а не
        // время. Механизм НЕ УСТАНОВЛЕН; кандидаты — неволатильный доступ к `tp_status`
        // mmap-кольца в `next_packet` (компилятор вправе кешировать) и расхождение позиций ядра и
        // читателя после переполнения.
        //
        // ЦЕНА НАЗВАНА: продукт идёт через `nfqueue`, этот захват в нём не задействован, и человек
        // сегодня беды не чувствует. Ложно лишь заявление `CanObserve` за пределами 256 кадров.
        // Решение владельца (05.09.2026): чиним, когда возьмёмся за reflex отдельно; тикет не
        // заводится намеренно, иначе он превратится в обещание себе, которое не исполняется.
