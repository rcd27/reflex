...
///
/// РЕЭКСПОРТ, НЕ ОПРЕДЕЛЕНИЕ (задача 12½): форма уехала в `reflex-engine::parse` — сам тип не
/// зовёт netlink, а тем, кто его строит здесь (приватная `tuple_of` рядом, не ссылка — имя не
/// экспортировано), и тем, кто его читает выше по стеку (`reflex-engine::parse::keyed_of_orig`),
/// ОДНО определение честнее второй копии одной формы.
/// Разбор дампа ctnetlink (ниже) не тронут — он заполняет те же поля так же, как заполнял.
pub use reflex_engine::parse::Tuple;
