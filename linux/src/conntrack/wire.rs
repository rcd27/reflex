...
///
/// РЕЭКСПОРТ, НЕ ОПРЕДЕЛЕНИЕ: форма живёт в `reflex-engine::parse` — сам тип netlink не зовёт, а
/// тому, кто его строит здесь (приватная `tuple_of` рядом, не ссылка — имя не экспортировано), и
/// тому, кто его читает выше по стеку (`reflex-engine::parse::keyed_of_orig`), ОДНО определение
/// честнее второй копии одной формы: копии расходятся молча, и сборка при этом зелёная.
