...
/// Сток категории — заявление, а не обёртка (канон §9): инжектор ВНЕ категории уходит вызовом
/// `send`, о котором ни один закон не знает, — приманки и извещения становятся эффектом без
/// свидетеля, а сертифицирован при этом чужой бэкенд. Команда — ГОТОВЫЕ БАЙТЫ:
