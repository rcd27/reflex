...
/// Сток категории — заявление, а не обёртка (#326, канон §9). До этого единственный боевой инжектор
/// стоял ВНЕ категории (сертифицирован был `AfPacketBackend`, которым не пользуются, а приманки и
/// извещения уходили вызовом `send`, о котором ни один закон не знал). Команда — ГОТОВЫЕ БАЙТЫ:
/// сериализацию делает [`CanInject::inject`](reflex_core::capability::CanInject::inject), сток
/// принимает то, что уже на проводе; принимай он
/// [`InjectablePacket`](reflex_core::command::InjectablePacket), выбор формы прятался бы внутри `emit`.
...
/// Ввод своего пакета — IP-формой, потому что сокет открыт с `IP_HDRINCL`. Канальная форма была бы
/// ложью на четырнадцать байт: ethernet-заголовок ядро дописывает само, отдай мы его — он уехал бы в
/// тело IP-заголовка.
