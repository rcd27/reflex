...
impl<'a> Iterator for Attrs<'a> {
    ...
    /// Выход один, и он гасит остаток: фьюзность держится тем, что отказ проходит ЕДИНСТВЕННОЙ
    /// веткой. Уйди обрыв через `?`, не тронув `self.rest`, — итератор вернул бы `None`, а на
    /// следующем шаге снова `Some` с теми же байтами.
...
///
/// Сборка — путь ЗАПИСИ: вердикт и конфигурация очереди, а с 14.09.2026 и забвение записи conntrack
/// (`Dump::forget`), потому под обеими фичами.
/// `test` — чтобы законы обратимости жили в этом же модуле.
#[cfg(any(test, feature = "nfqueue", feature = "conntrack"))]
