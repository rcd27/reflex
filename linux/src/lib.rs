...
//! Два крыла помечены устаревшими: `tc` и `tun`. Обе фичи включает только закрытая ветка первого
//! поколения; у живых потребителей в зависимостях лишь `nfqueue` и `conntrack`. Помечены, а не
//! снесены: там техника (eBPF-steering с картами действий, userspace-терминация TCP поверх smoltcp)
//! — полезное достанут разбором. Пометка атрибутом, а не только прозой: включивший фичу услышит
//! компилятор.
//!
//! ПОМЕТКА СНЯТА В ТЕСТОВОЙ СБОРКЕ (`cfg_attr(not(test), deprecated)`), и это не поблажка счёту.
//! Адресат пометки — тот, кто фичу ВКЛЮЧАЕТ ИЗВНЕ; собственные тесты крыла к нему не относятся, о
//! чём говорят и `#[allow(deprecated)]` на их модулях. Но `allow` внутри модуля до сгенерированного
//! тест-харнессом кода не достаёт: он обращается к тестам по пути через помеченный модуль и
//! предупреждает шестью строками, которых никто не заказывал. Снятие пометки под `test` лечит
//! причину — в обычной сборке, единственной, где пометку вообще кто-то читает, она на месте.
...
pub mod queue;
pub mod rawsend;
// Свидетель пути увода: спрашивает маршрутизатор ядра, куда уйдёт помеченное. Под `nfqueue`, ибо
// метку увода кладёт вердикт очереди — без неё свидетельствовать не о чем.
#[cfg(feature = "nfqueue")]
pub mod route;
...
#[cfg_attr(
    ...
    deprecated(
        since = "0.0.1",
        note = "фичу `tc` включает только закрытая ветка первого поколения; разбор техники отложен — см. заголовок lib.rs"
    )
)]
pub mod tc;
...
#[cfg_attr(
    ...
    deprecated(
        since = "0.0.1",
        note = "фичу `tun` включает только закрытая ветка первого поколения; разбор техники отложен — см. заголовок lib.rs"
    )
)]
pub mod tun;
...
// Функтор в категорию бэкендов (канон §9). Реализация аддитивна: inherent-методы остаются,
...
/// ключуется четырьмя байтами адреса, IPv6-поток невыразим. Молчаливый `Ok(())` означал бы
...
fn action_key(flow: &reflex_core::types::Flow) -> Result<u32, String> {
    match (flow.src.ip(), flow.dst.ip()) {
        ...
        | (std::net::IpAddr::V6(_), std::net::IpAddr::V4(_)) => Err(format!(
            "ACTION_TABLE ключуется IPv4, поток {} → {} невыразим",
            flow.src, flow.dst
        )),
    }
}
