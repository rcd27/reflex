...
/// Очередь netfilter как носитель. Дверь у неё ОДНА — вердикт удержанному пакету; наружу, к
/// постороннему собеседнику, она не говорит.
///
/// Оттого `CanAsk` она НЕ заявляет, и это сказано отсутствием impl, а не константным `None` в нём.
/// Заглушка тут стояла и была снята (09.09.2026): способность, всегда отвечающая «нечем», — не
/// способность, а обещание, которое никогда не исполняется. Пока она стояла, `Act::ask()` над
/// очередью СОБИРАЛСЯ и молча уходил ждать ответа, которого никто не пошлёт; теперь его не собрать
/// (§9.1 — гейт по акту). Носитель, у которого контур есть, объявит `CanAsk` и вернёт пакет-вопрос.
///
/// Различие, стоящее за этим: `None`, зависящий от ВХОДА (`CanSever::notice` — у датаграммы нет
/// формы обрыва), — рантайм по существу и остаётся; `None`, КОНСТАНТНЫЙ для носителя, — заглушка.
...
impl NfqueueBackend {
    ...
    /// Ждать на дескрипторе, а не крутиться: `recv()` в неблокирующем режиме отвечает «пусто»
    /// мгновенно, и цикл без ожидания зовёт его снова — замерено ~1100 холостых чтений НА ПАКЕТ,
    /// сожжённое ядро и непригодный замер цены обработки. Сам неблокирующий режим остаётся: без
    /// него цикл не проверит условие выхода и не кончится на пустой очереди.
    ...
    /// миру: слово ответа приезжает `Answered`, вердикт выбирается раз в `super::terminal`, а отказ
    /// ядра становится величиной (`NfqCounts::not_taken`), а не пропадает в `let _ =`.
