...
//
// ОЧЕРЕДЬ ВОШЛА В КАТЕГОРИЮ ТЕМ ЖЕ ВЕЧЕРОМ, но НЕ как `Source`, чего требовал снятый отсюда
// `TODO(#326)`. Проба дала `E0499`: `Source::packets(&mut self)` держит бэкенд заимствованным,
// пока жив поток, а ответ требует второго `&mut`, и обойти это нечем — netlink-сокет один, делить
// его, как `AfPacketBackend` делит кольцо и инжектор, не выйдет.
//
// Форма нашлась иная и по существу более точная: `Serves` (взять и ответить неделимо) плюс четыре
// способности над `Terminal` — `CanHold`/`CanRefuse`/`CanRewrite`/`CanMark`. Все четыре несут
// предмет и покрыты законами; см. `super::terminal`.
