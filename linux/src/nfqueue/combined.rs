...
// В КАТЕГОРИЮ НЕ ПОДКЛЮЧЁН, и это не забывчивость: у сочетания две калитки разной природы —
// кольцо `AfPacketBackend` (наблюдение, копия) и очередь (владение). Форма для такого сочетания
// не найдена, и выдумывать её раньше надобности значило бы заводить заявление без предмета.
