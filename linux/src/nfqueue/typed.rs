...
/// ПРОВОДНОЙ ПАКЕТ НЕСЁТ ФЛОУ — значит по нему можно держать состояние соединения примитивом
/// (`reflex_core::flow_table::FlowTable`), а не своей картой. Без этой реализации всякий
/// хендлер, которому нужна память о соединении, писал `HashMap<Flow, _>` руками — в неводе так
/// сделано пять раз, и ни разу с эвиктом по простою.
impl reflex_core::types::HasFlow for WirePacket {
    fn flow(&self) -> &reflex_core::types::Flow {
        &self.flow
    }
}
...
///
/// ПУБЛИЧНА РАДИ ЕДИНСТВЕННОСТИ ОПРЕДЕЛЕНИЯ. Потребитель, собравший TLS-запись из нескольких
/// сегментов, обязан классифицировать СКЛЕЙКУ — у него на руках байты, которых не было ни в
/// одном пакете. Своя копия этих трёх ветвей у потребителя молча разъехалась бы с этой при
/// первом же расширении `L7`, и разъезд не выразился бы ничем: обе стороны компилируются.
pub fn classify_l7(l7: &[u8]) -> L7 {
