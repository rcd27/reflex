...
/// # Чем оплачено
///
/// Лечение, прошедшее полную приёмку на стенде, на коробке отвечает `Kernel(-22)`: в её образе нет
...
///
/// Опись и есть шаг предпосылок, закрывающий разрыв между стендом и коробкой: одна строка
