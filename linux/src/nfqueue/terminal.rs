...
//! (канон §9). Решение отделено от эффекта: цепочка порождает
//! [`Answered`](reflex_core::held::Answered), мира касается ровно [`Terminal::apply`], а отказ ядра
//! (например, по таймауту очереди) становится [`Refused`](reflex_core::held::Refused) — величиной,
//! которую можно записать, сравнить и развернуть, в отличие от вызова, чей результат некуда
//! положить. `&mut` остаётся (netlink-сокет один, отправка последовательна по природе), но стоит В
//! ОДНОМ месте: выше значения, ниже мир.
...
// Заявления очереди — четыре на четыре слова алфавита, покрытие полное. До этого
...
/// Обрыв — пара, и теперь он один предмет. Сборка `RST` жила в продукте, определение обрыва
/// существовало трижды (полностью в цепочке потребителя, наполовину в примере движка, никак во
/// втором бинаре) —
...
/// Четвёртое слово обрело предмет: оставалось без способности (метку не видно на проводе),
...
///. `Source::packets(&mut self)` держит бэкенд заимствованным, пока жив поток, а ответ
