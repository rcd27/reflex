...
> {
    ...
    // Это НЕ второй читатель предпосылки вместо задачи 7: запрет был про двух читателей В ОДНОЙ
    // цепочке (`QueueSocket::open` рядом с `Nfqueue::open`, где значения могли бы разойтись между
    // двумя чтениями одного прогона). Здесь `Nfqueue::open` вообще нет — у этого бинаря свой корень
    // композиции (`--features certify`, отдельная программа), и корень, читающий свою предпосылку
    // сам, применяет закон `TimeoutBase::read`, а не дублирует его вторым законом об одной величине.
