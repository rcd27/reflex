...
///
/// Берётся из ДОМА (`core::mark`), а не объявляется здесь числом: прежде оно стояло тремя копиями
/// — тут, в фасаде и в скрипте стенда, — и разошлись бы они молча, а узналось бы это по тому, что
/// впрыск возвращается в свою очередь.
fn ours() -> u32 {
...
> {
    ...
    // про двух читателей В ОДНОЙ цепочке (`QueueSocket::open` рядом с `Nfqueue::open`, где значения
    // могли бы разойтись между двумя чтениями одного прогона). Здесь `Nfqueue::open` вообще нет — у
    // этого бинаря свой корень композиции (`--features certify`, отдельная программа), и корень,
    // читающий свою предпосылку сам, применяет закон `TimeoutBase::read`, а не дублирует его вторым
    // законом об одной величине.
    let base = TimeoutBase::read().ok_or_else(|| {
        "нет базы таймаутов conntrack: включи nf_conntrack_acct и nf_conntrack_timestamp"
            .to_string()
    })?;
    let mut socket = QueueSocket::open(number, base).map_err(|why| format!("сокет: {why:?}"))?;
