#!/bin/sh
# Внутри контейнера: движок на очереди + реальный трафик через реальный ТСПУ линии замера. Цель —
...
if [ "$GOT" = 1 ] && [ "$CONTROL_HIT" = 0 ]; then
  echo "[итог] ЗЕЛЕНО: IP-blackhole «$TARGET» пойман по адресу на реальном трафике; «$CONTROL» чист"
  exit 0
fi
echo "[итог] КРАСНО"
exit 1
