#!/bin/sh
# Проверка на реальном трафике с хоста: собрать образ, прогнать движок на РЕАЛЬНОМ трафике линии замера,
# пробросить вердикт. Требует: docker, линия за ТСПУ (иначе цель не роняется — см. README).
set -eu
cd "$(dirname "$0")"
