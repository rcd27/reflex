#!/bin/sh
# Внутри контейнера: движок на очереди UDP:53 + настоящие DNS-запросы через реальный ТСПУ линии замера.
# Цель — домен со стабильным инжектом NXDOMAIN; контроль — домашний домен (настоящий ответ).
set -u

TARGET=${TARGET:-nnmclub.to}   # стабильный инжект NXDOMAIN @8.8.8.8 на линии замера
