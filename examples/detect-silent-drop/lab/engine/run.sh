...
# 3. Реальный трафик. Цель повиснет (max-time держит соединение дольше окна тишины 5с);
...
if [ "$SUSPECT" = 1 ] && [ "$CONFIRM" = 1 ] && [ "$CONTROL_HIT" = 0 ]; then
  echo "[итог] ЗЕЛЕНО: «$TARGET» пойман обоими приборами на реальном трафике; «$CONTROL» чист"
  exit 0
fi
echo "[итог] КРАСНО"
exit 1
