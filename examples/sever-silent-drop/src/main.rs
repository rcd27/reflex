...
//!
//! `Act::sever()` — не вариант перечисления, а КОНСТРУКТОР, и он существует лишь над носителем,
//! умеющим обрывать (§9.1). Способность требуется по акту: наблюдение не просит ничего сверх
//! удержания, обрыв просит `CanSever`, вопрос — `CanAsk`. Очередь netfilter спрашивать не умеет,
//! и `Act::ask()` в этой цепочке просто не соберётся — вместо цепочки, молча ждущей ответа,
//! которого никто не пошлёт.
//!
