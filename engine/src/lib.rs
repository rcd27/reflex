...
// `#![no_std]` ЗДЕСЬ ДИСЦИПЛИНА СТИЛЯ, А НЕ СВОЙСТВО СБОРКИ — названо вслух, чтобы атрибут никто не
// прочёл как гарантию: зависимость `reflex-core` → `futures` тянет `std` в сборку помимо нас, и
// прогона, способного покраснеть от `std` в этом крейте, не существует (§10.7). Безусловность
// оплачена двумя унаследованными типами: разбор провода строит `Flow` через `std::net::SocketAddr`
// (тип из `reflex-core`, который `std` и не прячет), а таблица разговоров (`Talks`) держит
// `BTreeMap`. Дисциплина держится для СОБСТВЕННОГО кода модулей, чужих типов не наследовавших:
// `interpret`, `meter`, `row`, `step`, `watch`.
extern crate std;

pub mod interpret;
pub mod meter;
pub mod name;
// РАЗБОР ПРОВОДА ЖИВЁТ ПРИ МАШИНЕ, А НЕ ПРИ НОСИТЕЛЕ. Код ЧИСТЫЙ: не зовёт netlink, не знает
// очереди, и соседей, тянущих Linux, у него здесь нет. Дом при носителе зависел бы от
// `reflex-linux` (сырой `AF_NETLINK`, крейт `nfq`, не собирающийся на `windows-msvc`) и валил бы
// кросс-сборку фасада чужим законом вместо своего — а носитель есть параметр, и всё ниже головы
// цепочки от него не зависит (Утв. 11.2).
//
// Законы разбора стоят там же, в `engine/tests/`. Исключение одно: закон согласия проводного и
// ядерного ключа живёт в `linux/tests/flow_identity.rs`, ибо его предмет не разбор, а СОГЛАСИЕ
// двух источников, и знает обоих только тот крейт.
pub mod parse;
pub mod row;
pub mod step;
// ТАБЛИЦА РАЗГОВОРОВ — по той же причине, что и `parse`: знает только `Dir`/`Flow` (отсюда же) и
// словарь провода (`reflex-instrument`), Linux не знает.
pub mod talk;
pub mod watch;

// Адрес живёт в `reflex-core`: им ключуется область цели (`Base::Fibre`, §4), а ключ области не
// может ссылаться ВВЕРХ по зависимостям. Здесь — реэкспорт, не второй дом.
...
// то, чем она расслаивается (§4). Отпечаток `FlowKey(u64)` отвергнут как лосси: две четвёрки с
// одним хэшем дают один слой, то есть одну машину на два разговора, и молча. Цена отказа замерена:
...
/// Что пакет сказал о личности цели. План, разрешаемый на `Cursor::Fresh` (SYN), знает о цели один
/// адрес — и на общем адресе CDN знание накрывает 69 посторонних имён. Круг разрывается переносом
/// МОМЕНТА, не таблицей адрес→имя (она врёт, где имён десятки, и её
...
/// План для пакетов разговора. Отвода здесь НЕТ: исполнителя не существует (`edge-quic` написан и
/// не подключён), а вариант без исполнителя делал бы законным состояние «принято и не применено» —
/// тот же довод, по которому не заявляется способность, постоянно отвечающая `None` (правило 9.5).
/// Пока исполнителя нет, отвод НЕВЫРАЗИМ, а не молчаливо неисполнен:
///
/// ```compile_fail
/// // Отвода нет вместе с исполнителем: назвать его нечем.
...
pub struct Run {
    ...
    /// знание (оно растёт join'ом в плоскости). Второй оси (`Standing { Provisional, Settled }`)
    /// здесь нет намеренно: её произведение с личностью держало бы представимыми «принято по
    /// личности, которой ещё нет» и «ждём имени, которого не будет» — незаконные состояния.
    ...
    /// Приказ, ждущий пакета. Не `Option<Programme>`: приказ на живой разговор ровно один по
...
pub enum Cursor {
    ...
    /// переживает закрытие, и это оплачено: с пустым вариантом `Run` выбрасывался, и `node_of`
    /// красил всякий завершённый разговор в `Told::Blind` (§7) — 83 слепые цели из 86. Ответ —
    /// единственное,
...
pub enum Sighting {
    ...
    /// Приветствие прошло — разговор узнал, кто цель. Момент, в который личность становится
...
// Печать судьбы личности стоит методом у типа (`Disclosed::rung()`), а не `Display`: `impl Display
// for row::Naming<()>` здесь незаконен (E0117 — и трейт, и тип чужие), а печатать ступень, молча о
// значении, — ловушка для `Disclosed<String>`.

/// Улика, а не только имя. Паспорт даёт ИМЯ события (довольно метрике); делу разметчика
...
/// знание, записанное дважды, расходится молча. Единица времени названа в имени поля (`_ns`):
/// `Span` — разность тиков, кормящий
...
/// запросом: восстанавливать имя на стороне потребителя запрещено (второй источник
...
impl reflex_instrument::Instrument for SightingInstrument {
    ...
    /// Публичные имена событий — реестр, снятый с типа: `Debug` сменил бы метку молча. Имя на
    /// девять исходов одно быть не может — различающая способность, потерянная в последнем шаге,
    /// неотличима от отсутствующей. Реестр стоит ЗДЕСЬ, у типа, и журнал зовёт паспорт: знание,
    /// записанное дважды, расходится молча.
...
mod sighting_passport_tests {
    ...
    /// Улика открывается паспортным именем. Разойдись слова — публичный круг получит два
