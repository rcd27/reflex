...
// Только под тест: крейт `no_std` без `alloc` и остаётся таким, но проверка улики (#325) сравнивает
// строку, а строку без кучи не собрать. `cfg(test)` живёт ровно столько, сколько прогон.
#[cfg(test)]
extern crate std;
...
/// байты из буквы недостижимы — кому нужно содержимое, разбирает ДО, в краю (`engine-nfq::parse`).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Packet {
    pub flow: FlowKey,
    pub dst: Addr,
...
/// К чему относится наблюдение (§4). Буквой, а не `FlowKey(0)`: наблюдение о ЦЕЛИ разговора не имеет
