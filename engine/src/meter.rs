//! Счёт цели и машины. Кольцо временны́х корзин живёт в фундаменте — это механизм измерения темпа
//! по окну, нужный ВСЯКОМУ наблюдателю потока, и нашего предмета он не знает. Здесь — обёртки,
//! которые его знают: `Target`, `Tally` и правила начисления по `Packet`/`Run`.
...
/// Счёт машины — молчащий наблюдатель горячего пути. Слово `()` (счёт не адресован никому,
