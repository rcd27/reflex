...
//! стоять внизу, у провода, а не подниматься в домен решений. Прежняя редакция жила в другом
//! крейте — то есть в КРАЮ движка, хотя ни одного обращения к сети не делала.
//!
...
//! ЭТО В СЧЁТ** (какой ноге приписать беду) живёт в другом крейте, в политике, потому что нога
...
//! `instrument` ← движок ← потребитель, ни одной стрелки назад.
//!
...
//! * ПОТЕРЯ ЦЕЛИ сюда не доходит вовсе, и это сильнее прежнего `None`: перевод берёт наблюдения
//!   РАЗГОВОРА, а потеря адресована цели и живёт отдельным словом. Записать собственную
//!   забывчивость в свойство ноги теперь нечем — не потому, что мы помним об этом, а потому, что
//!   такой вызов не собирается.
...
pub fn distress(sighting: &Sighting) -> Option<Distress> {
    match sighting {
        ...
        // НАША СЛЕПОТА И СЛУЖЕБНЫЕ СОБЫТИЯ. `Stale` — план устарел; `Opened`/`TargetSpoke` —
        // начало разговора и ответ цели, то есть скорее отсутствие беды. `Peaked` — темп, для
        // которого нужен недоказанный порог (см. шапку).
        //
...
mod tests {
    ...
    ///
    /// НОГИ ЗДЕСЬ НЕТ, и это предмет отдельного витнеса выше по лестнице, в политике: потеряй мы
    /// имя ноги ТАМ, беда на обходе понизила бы оценку прямого пути. Здесь же нога не участвует
    /// вовсе — распознавание от неё не зависит.
    #[test]
    ...
    /// Записать их в свойство ноги значило бы учиться на своей забывчивости.
    ///
    /// Потери цели здесь нет намеренно: она адресована цели, а не разговору, и в этот перевод
    /// больше не входит — компилятором, а не памятью автора.
    #[test]
