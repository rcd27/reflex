...
fn noted(run: &Run, packet: &Packet<'_>, now: Tick, what: Sighting) -> Noted {
    Noted {
        ...
        // БУКВА АДРЕСАТА СТАВИТСЯ ЗДЕСЬ ЖЕ: всё, что видит шаг, видит РАЗГОВОР — потеря цели
        // случается не на пакете и через эту дверь не проходит.
...
///
/// ЦЕНА ЭТОГО СВЁРТЫВАНИЯ НАЗВАНА: свободная функция зовёт `look_up` ДВАЖДЫ за разговор, разными
/// ключами (на открытии — по адресу, на приветствии — по имени), а одна буква двух разных ответов
/// не выражает. Продукт их уже свёл — `Plane::feed` подаёт постоянное замыкание, — и потому лифт
/// ему верен; но морфизм, которому понадобятся оба ответа, потребует второй буквы, а не второго
/// Reader'а.
///
...
///
/// # ПОДНЯТА ПОЛОВИНА СОСТОЯНИЯ, И ЭТО СКАЗАНО
///
/// Продукт гонит `advance(step, counted, cursor, tally, …)`, то есть машину над ПАРОЙ
/// `(Cursor, Tally)`. Здесь поднят курсор; счёт остаётся снаружи. Решение это верное по объёму —
/// плоскость расслаивается отдельной работой, — но молчать о нём нельзя: из счёта растёт
...
impl<'a> Advancing<'a> {
    ...
    /// `Plan`, ровно как в продукте: `Plane::feed` берёт `epoch = plan.epoch` из того же поиска,
...
/// сразу — то есть пришлось бы назвать область и наблюдению. Своей у него нет: наблюдение копится,
/// а не решает ничью судьбу. Имя переносит объявление на ОТВЕТ целиком, и вот это уже верно: ядро
/// держит пакет, пока ответ не дан, и обе половины уезжают на нём.
///
/// ОГОВОРКА НАЗВАНА: сосед по стрелке видит здесь и наблюдение — тип этого не запрещает, и
/// запретить сможет только второй выход шага, идущий вбок.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Answer {
    pub act: Act,
