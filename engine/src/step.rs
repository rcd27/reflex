...
// TODO: числа объявлены ПО СЛУХУ и не замерены — предел назван здесь, чтобы бюджет не читался
// как показание. Заменить замером на риге.
...
/// `look_up` зовётся дважды за разговор: на открытии — по адресу (SYN), на пакете, где цель
    ...
    L: Fn(&Packet) -> Plan,
{
    ...
    // Устаревание — функция состояния, а не след события: след (`&& !stale_told`) неидемпотентен,
    // и потеря отменяла бы приказ навсегда. Спрашивается эпоха, повторы дросселирует
    ...
    Stepped {
        ...
        cursor: match (run.ordered, over) {
            ...
            // `Fresh`: иначе у `Fresh` два смысла и теряется ответ цели (83 слепые цели из 86
...
/// Положить в разговор приказ оборвать. Приказ адресован РАЗГОВОРУ, не цели: план замерзает
