...
/// (`CTA_TUPLE_ORIG`). Форма живёт ЗДЕСЬ, вместе с [`keyed_of_orig`], единственным её
/// потребителем: сам тип не зовёт netlink и не знает ядра — пять плоских полей, разбор ctnetlink
/// лишь ИХ ЗАПОЛНЯЕТ, и соседей, тянущих Linux, у формы тут нет.
/// `reflex_linux::conntrack::wire::Tuple` — реэкспорт ОТСЮДА, а не вторая копия формы: один
/// предмет — один закон, и две копии разошлись бы молча.
...
/// держал бы расщепление истинным в коде и невыразимым в типе.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// в личность ВХОДИТ: разговор по QUIC и разговор по TCP — разные разговоры. Слить их одним
/// ключом, чтобы знание о цели не разъехалось по транспортам, значит взять у личности разговора в
/// долг для нужд цели: цель есть отдельный слой (`TargetKey` протокола не несёт), и сводит
/// разговоры копредел ПО СЛОЮ (§4), а не общий ключ.
