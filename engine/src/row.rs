...
pub enum Watched {
    ...
    Seen {
        ...
        /// либо `Silent`). Отдельного `Standing` здесь нет: вторая ось держала бы незаконные
        /// клетки представимыми.
        naming: Said,
        counted: Counted,
        answered: Answered,
    },
...
/// Узел цели — то, что человек видит подстрокой. Личности здесь нет, и это разобрано: поле
...
/// добычи счёта ядра, netfilter): standalone-показ собирается и поверяется на машине без
/// conntrack. Половина предмета, доступная одной ОС, есть случайность истории, не граница слоя.
...
/// выборкой из мультимножества (так погибло поле личности). Счета складываются, ответ ранний
...
/// Тип строки читается без netfilter. Проверка живёт в `dataplane`: крейт `no_std` без
...
mod row_needs_no_kernel_tests {
    ...
    /// выборкой из мультимножества (так погибло поле личности); проверка красит следующее.
    #[test]
    fn merging_does_not_depend_on_the_order_the_kernel_listed_them() {
        ...
        // TODO: `watched` — ВЫБОРКА, а не свойство узла. Узел агрегирует разговоры, `Watched`
        ...
        assert_ne!(
            ...
            "выборка перестала быть выборкой — снять `TODO` и сравнивать узлы целиком"
        );
    }
