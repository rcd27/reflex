...
//! нужен ядру, показание — приборам). Перевод внутри `Plane` край взять не может и заводит свой;
//! две памяти расходятся молча, и это оплачено дважды: 1136 флоу из 1136 мимо, 16 повторов вместо
//! 6 на `sag`. Правило сторон сюда не входит: кто клиент — вопрос ВХОДА (порт у очереди, улика у записи),
...
/// продолжение потока начинается ровно с границы, и «не дальше» считало бы его повтором — 16
/// вместо 6 у `tshark`. Расстояние с обёрткой: `seq` 32-битен, прямое `<` соврало бы раз за переполнение.
    ...
    match (wire.resets, wire.opens, wire.handshakes) {
        ...
        (true, _, _) => Some(SeenTcp::Rst {
            by: match from_client {
                ...
                // имени, а различитель (TTL, фингерпринт) здесь не читается.
                false => ResetBy::TargetSide,
            },
        }),
        // Стук клиента и ответ цели — РАЗНЫЕ события: дай обоим `Syn` — «рукопожатие состоялось»
        // станет невыразимо, и блокировка по адресу сольётся с блокировкой по имени.
        ...
        // Прощание — только на сегменте без данных: правило «одно событие на сегмент»
...
/// Проверки стоят при поведении, а не при его потребителе: покрытие, оставшееся в чужом крейте,
/// стережёт вторую реализацию перевода — либо исчезает молча вместе с нею.
#[cfg(test)]
mod tests {
    use super::*;
