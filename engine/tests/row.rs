//! СТРОКА ЦЕЛИ: пять законов вместо семнадцати случаев.
//!
//! Имена здесь — законы, и названы верно: «пустой счёт нейтрален», «счёт не зависит от порядка»,
//! «счёт не зависит от группировки» суть аксиомы коммутативного моноида. Но закон алгебры,
//! предъявленный ОДНОЙ тройкой значений, есть обещание, а не закон: `added` волен ломать
//! ассоциативность у границы `u64` (насыщение — то самое место, где алгебра обычно и рвётся), и
//! три примера этого не увидят.
//!
...
/// СЧЁТ ОТ СЛИЯНИЯ НЕ УБЫВАЕТ — закон, которого у моноида нет.
///
...
///
/// Проверяется на ВСЕХ адресах набора и ОБЕИХ ширинах, а не на одном адресе.
...
fn an_unnamed_target_is_keyed_by_width_and_an_awaited_name_does_not_change_the_row() {
    ...
    assert_eq!(
        net_of(Addr(0x0A_0B_0C_0D)),
        Addr(0x0A_0B_0C_00),
        "сеть срывает младший байт"
    );
    assert_eq!(
        host_of(Addr(0x0A_0B_0C_0D)),
        Addr(0x0A_0B_0C_0D),
        "хозяин — весь адрес"
    );
}
