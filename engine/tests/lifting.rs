...
///
/// ТРОЙКА, А НЕ `Answer`: слитый тип вердикта с наблюдением возник бы лишь оттого, что голому
/// кортежу не объявить адреса, не соврав про половину — `Act` адресован пакету, `Noted` не
/// адресован никому. Второй выход шага обёртки не требует.
#[test]
...
///
/// Цикл с `Packet<'a>` НЕ СОБИРАЕТСЯ (`E0597`), и мимо морфизма пришлось бы звать свободную
/// функцию: лайфтайм уходит в `Advancing<'a>`, а `Mealy::In` не умеет заимствовать только на время
/// вызова — все пакеты одной машины делили бы одну область заимствования, тогда как на живом
/// носителе байты
/// принадлежат сообщению ядра и живут до вердикта.
///
/// Байты здесь заводятся ВНУТРИ оборота намеренно: это и есть форма цикла над живым носителем, а не
/// украшение теста.
#[test]
