...
///
/// Пара тестов, и проверяется она РАЗНИЦЕЙ: одно значение не установило бы ничего, ибо обе
/// величины под одним именем `Lost` делят и один акт `Pass`. Цена слияния названа числом: витнес
/// `plane-mark.sh` краснел на 2 прогонах из 10 — последний `ACK` уходил мимо ноги при выученной
/// цели.
#[test]
...
/// Знание спрашивается ДВАЖДЫ за разговор и не чаще: на открытии и на назывании. Не «однажды» —
/// но и не на каждом пакете: спроси мы на каждом, страта менялась бы посреди применения.
#[test]
...
/// ГЛАВНЫЙ ЗАКОН: знание, ключёванное ИМЕНЕМ, доезжает до соединения, которое на открытии было
/// безымянным. План, взятый на SYN, брался бы по адресу — единственному, что известно в тот миг, —
/// и второй адрес той же цели уходил бы мимо ноги: «пакетов ногой 0» при исправной петле, живой
/// прогон.
#[test]
...
///
/// Предмет проверяется МЕТКОЙ, а не отводом: свойство принадлежит не отводу, а всякому решению,
...
/// ПРИВЕТСТВИЕ БЕЗ ИМЕНИ ЗАКРЫВАЕТ ВОПРОС О ЛИЧНОСТИ, А НЕ ОТКЛАДЫВАЕТ ЕГО.
///
/// Жди принятие плана ИМЕНИ — и разговор, чьё приветствие прошло без имени, остался бы «временным»
/// навсегда. Мелькни в нём позже имя — страта сменилась бы на середине применения, то есть
/// применилась бы полторы.
#[test]
...
// ═══ ЗАКОНЫ JOIN'А ЗНАНИЯ О ЛИЧНОСТИ (§5) ═══
//
...
/// ПРИКАЗ ОБОРВАТЬ ЖИВОЙ РАЗГОВОР ИСПОЛНЯЕТСЯ И ПОДТВЕРЖДАЕТСЯ.
///
/// Замысел владельца дословно: «движок говорит датаплейну — сбрасывай такое-то соединение со стороны
...
///
/// Живи `Said` только внутри шага — план по нему принимался бы, а наблюдения не выпускалось: для
/// потребителя двери момент, в который цель стала известной, не наступал бы вовсе, — а он
/// единственный, где ключ цели доезжает вовремя.
#[test]
