...
//!
//! Композиция живёт в фундаменте (`Pair`), и здесь проверяется НАСТОЯЩАЯ пара продукта:
//! `Advancing` говорит вердикт, `Counting` молчит и копит. Свободная функция
//! `advance(step, counted, …)` — рукописная композиция «посчитать и шагнуть» — на эту роль не
//! годится: проверять её можно лишь подставными шагами-замыканиями, то есть на том, чего в цепочке
//! потребителя нет.
//!
//! Три свойства — «счёт идёт при потере, при отводе, при обрыве» — держатся ПОДПИСЬЮ, а не тестом:
