...
/// ОБЯЗАТЕЛЬСТВО ПЕРЕД ЛИФТОМ: флоу остаётся под наблюдением, пока его личность в ⊥. Проверяется ПО ВСЕМУ пространству планов, а не на одном:
...
///
/// Одного случая мало, и это не педантизм: проверка одного `Mark` на обезоруженном разряде отвода
/// НЕ краснеет — отвод без него читается обратно как обычный маршрут `Mark(0x01)`, то есть
/// разговор едет не туда, а прибор соглашается. Зрячесть проверки обязана быть показана на всём
/// пространстве решений (§10.7), не на образце.
///
/// Отвода в этом пространстве нет, пока нет исполнителя, — урок стережёт то, что есть:
/// заведётся отвод, вернуть сюда и его случаи.
#[test]
