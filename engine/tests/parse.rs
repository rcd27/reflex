...
///
/// Требование совпадения ключей верно по НАМЕРЕНИЮ: разойдись знание о цели по транспортам —
/// лечение, заработанное на TCP, не досталось бы QUIC. Но место намерению не то. Сводить транспорты — работа слоя ЦЕЛИ (`TargetKey` протокола не несёт), а не
