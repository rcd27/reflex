...
/// ПРЕДМЕТ: область, которую дверь называет наружу ([`Telling::region`]), — ТА САМАЯ, которой она
/// меряет решения.
///
/// Дверь публичная и до этого закона не вызванная ни разу: её звал только движок при постройке.
/// Потребителю она нужна ровно затем, чтобы согласовать СВОИ области с нашей, — и отдавай она не
/// то, согласование выглядело бы сделанным, а движок всё равно отказал бы (или, что хуже, не
/// отказал и пустил бы двух писателей на одни биты).
///
/// Закон связывает ОТВЕТ с ПОВЕДЕНИЕМ: ширина берётся из ответа двери, и по ней же проверяется,
/// что решение влезает, а на бит шире — нет. Геттер, сверенный сам с собой, ничего бы не стоил.
#[test]
fn the_region_a_door_names_is_the_one_it_measures_decisions_by() {
    let telling = Telling::over(leg());
    let named = telling.region();

    // Самое широкое значение, влезающее в названную область, и первое, не влезающее.
    let widest = named.mask() >> named.mask().trailing_zeros();
    assert!(
        telling.tell("example.com", widest),
        "значение шириной ровно в названную область обязано лечь: {widest:#x} в {:#x}",
        named.mask()
    );
    assert!(
        !telling.tell("example.com", widest + 1),
        "значение шире названной области не кладётся вовсе — обрезать молча значило бы подменить \
         решение соседним"
    );
}
