...
///
/// Замер потребителя (19.09.2026): DNS-пайп на боевой машине даёт 0,6 пакетной буквы в минуту — 64
...
fn a_chain_whose_machines_fell_silent_says_so_instead_of_saying_nothing() {
    ...
    engine(
        Paper::new()
            ...
            .then_packet(request(40001))
            // Дальше провод молчит, как молчит редкая очередь между наблюдениями.
            .silent_for(secs(30))
            .then_stop(),
    )
    .from(Tcp)
...
/// ПРЕДМЕТ: лента не растёт сверх окна, даже когда узлов сетки в потоке нет.
///
/// Найдено замером цены двери (`bench_throughput.rs`): на корпусе в 2000 кадров, уместившемся в
/// одну десятую секунды, свидетельство пришло ОДНО — из 2000 букв. Окно в 64 буквы проверялось
/// только на границе узла сетки, а узлов при плотном времени не случается вовсе: `TICK` 200 мс, а
/// весь корпус короче.
///
/// Цена такой ленты не в опоздании вердикта, а в памяти: она хранит КЛОН слова провода на каждую
/// букву и растёт до конца источника. Докблок обещал обратное («иначе лента росла бы вечно») —
/// обещание держалось на том, что узлы приходят, то есть на свойстве живой очереди, которого у
/// записи нет.
#[test]
fn the_tape_does_not_outgrow_its_window_when_no_grid_node_falls() {
    let (tx, heard) = sync_channel(256);

    // Запись, целиком уместившаяся между двумя узлами: моменты идут микросекундами.
    let mut paper = Paper::new().then_packet(syn(40001));
    for _ in 0..300 {
        paper = paper.then_packet(request(40001));
    }
    ...
    assert!(
        said.len() >= 4,
        "триста букв без единого узла обязаны закрыть несколько окон по 64, а не копиться одной \
         лентой до конца источника; свидетельств пришло {}",
        said.len()
    );
}
