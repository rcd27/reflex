...
use paper::{alien, log, names, request, syn, taken, Crier, Letter, Paper, PaperAnswer, Recorder};
use reflex::*;
...
fn отказ_вердикта_не_отменяет_прошедших_букв() {
    ...
    assert_eq!(names(seen), ["packet"], "буквы прошли до отказа");
}
...
/// E2. ДЫРА НЕСЁТ МОМЕНТ ОБНАРУЖЕНИЯ, а не потери: момента потери не существует нигде — ядро
/// выбросило сообщения раньше, чем мы позвали приём, и `ENOBUFS` не несёт ни числа потерянных, ни
/// времени. Оттого буква не может лечь РАНЬШЕ последнего наблюдения: всё, что прошло между
/// потерей и её объявлением, приборы уже приняли за полное наблюдение.
#[test]
fn дыра_несёт_момент_обнаружения_а_не_потери() {
    let seen = log::<Letter>();
    let paper = Paper::new()
        .then_packet(request(40001))
        .silent_for(Duration::from_secs(1))
    ...
    let before = &letters[letters.len() - 2];
    assert!(
        torn.at >= before.at,
        "дыра объявлена не раньше последнего наблюдения"
    );
}
