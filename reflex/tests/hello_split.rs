...
/// на такой длине» от «на той линии случилось что-то другое».
#[test]
...
/// ПРЕДМЕТ, ПОЙМАННЫЙ ЗАМЕРОМ ПОТРЕБИТЕЛЯ: ХВОСТ ПРИШЁЛ РАНЬШЕ ГОЛОВЫ.
///
/// На живом проводе порядок не обещан никем: голова приветствия могла быть дропнута до нашего хука и прийти
/// лишь повтором — уже после хвоста. Прежде первый клиентский сегмент с данными, каким бы он ни
/// был, СТАВИЛ ОСНОВАНИЕ сдвигов; хвост, пришедший первым, отравлял его навсегда, голова получала
/// сдвиг «назад» (обёртка номера — около четырёх миллиардов), потолок её отбрасывал, и имя не
/// поднималось НИКОГДА.
///
/// Симптом у потребителя был именно такой: на ЗАПИСИ той же беседы имя поднималось, на живом
/// проводе — ни
...
fn a_tail_that_arrived_before_the_head_does_not_bury_the_name() {
    ...
    let said = named(
        Paper::new()
            ...
            // Хвост первым — так его увидела очередь, когда голова не дошла.
            .then_packet(segment(40005, 1 + first.len() as u32, tail))
            // Голова повтором, уже после.
            .then_packet(segment(40005, 1, first))
            .then_packet(segment(40005, 1 + first.len() as u32, tail))
            .then_stop(),
    );

    assert!(
        said.iter().any(|name| name == "instagram.com"),
        "голова, пришедшая повтором после хвоста, обязана дать имя; названо: {said:?}"
    );
}
