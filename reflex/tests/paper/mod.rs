...
//!
//! КАЖДЫЙ ИНТЕГРАЦИОННЫЙ ТЕСТ, ОБЪЯВИВШИЙ `mod paper;`, КОМПИЛИРУЕТ ЭТОТ ФАЙЛ ЗАНОВО В СВОЁМ
//! БИНАРНИКЕ (`driver.rs`, `own_word.rs`) — общей сборки у `tests/*.rs` нет. Оснастку, которую
//! исчерпывает один бинарник (`driver.rs` гоняет по ней всю таблицу шва T8), другой (`own_word.rs`,
//! которому нужны только `log`/`request`/`Paper`) видит мёртвой: `dead_code` судит по СВОЕМУ
//! бинарнику, не по сюите целиком. `#![allow(dead_code)]` ниже — поправка на способ счёта, а не
//! индульгенция мёртвому: живое здесь то, что востребовал `driver.rs`, и проверяется это его же
//! прогоном (`cargo test -p reflex --test driver`), а не грепом по этому файлу.
