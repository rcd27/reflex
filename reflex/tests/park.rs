...
/// ПРЕДМЕТ: «слушает ли кто-нибудь эту очередь» спрашивается ЧЕРЕЗ ФАСАД, а не мимо него.
///
/// Заказано третьим неводом (#340): подъём слота десинка свидетельствуется тем, что к очереди
/// привязан сокет, — и свидетель здесь ядро, а не тот, кто запускал слушателя. Оплачено парком
/// 15.09.2026: движок умер на старте, а продукт сутки печатал «нога несёт», потому что
/// свидетельством считался успешный `spawn`, то есть ОБЪЯВЛЯЮЩИЙ.
///
/// Без этой двери потребителю пришлось бы либо тянуть линукс-крейт мимо фасада (та самая течь, за
/// которой стоит сторож соседним законом), либо читать `/proc` своей рукой — то есть завести
/// ВТОРОЙ источник правды об одном предмете.
///
/// Проверяется здесь ровно то, что проверяемо без ядра под рукой: дверь открыта, отвечает КЛЕТКОЙ
/// и не объявляет слушателя там, где его нет. Что строка очереди и есть слушатель — закон разбора,
/// и он стоит парой там, где живёт разбор (`linux/src/queue/socket.rs`).
#[test]
fn the_facade_can_ask_whether_a_queue_has_a_listener() {
    // Номер, которого не бывает у живых правил: строки для него в файле нет ни на одной машине.
    let answer = reflex::Nfqueue::listened(65535);

    assert!(
        !matches!(answer, Some(true)),
        "слушатель объявлен у очереди, которой никто не слушает: {answer:?}"
    );
    // `None` — не «не слушает», а «не смотрели»: файла нет, модуль не загружен, машина не та (§7).
    // Обе клетки законны, и тест не требует ядра — он требует, чтобы дверь была.
    assert!(matches!(answer, None | Some(false)));
}
