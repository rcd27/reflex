...
///
/// Предмет заказан случаем, где соседний процесс садится на очередь, а знать о нём нужно нам:
/// свидетель привязки — ядро, а не тот, кто запускал слушателя. Оплачено полем 15.09.2026: процесс
/// умер на старте, а наблюдатель сутки докладывал об успехе, потому что свидетельством считался
/// успешный `spawn`, то есть ОБЪЯВЛЯЮЩИЙ.
///
