...
/// ПРЕДМЕТ: утрата по потолку разговоров приходит ЗНАЧЕНИЕМ ([`Report::forgotten`]).
///
/// Дверь публичная и до этого закона не вызванная НИ РАЗУ: о ней говорили докблоки — в том числе
/// соседний закон о `parting`, объясняющий, почему на боевом пути её мало, — а спросить у отчёта
/// число никто не пробовал. Между тем именно она отличает «движку стало тесно» от «в сети тихо»:
/// разговоры, снятые ЖИВЫМИ, приборы не проводили, и молчание о них читается как здоровье.
///
/// Носитель здесь конечный (сценарий), и это не удобство теста, а единственное место, где дверь
/// достижима: `Report` рождается по концу источника, которого у очереди ядра не бывает.
#[test]
fn the_report_counts_conversations_dropped_alive_by_the_ceiling() {
    // Разговоров заведомо больше потолка, и все живые: пауз в сценарии нет, снять их по сроку
    // цикл не может — значит снимет по потолку, и только по нему.
    let mut paper = Paper::new();
    for n in 0..(reflex::CONVERSATIONS + 200) {
        paper = paper.then_packet(request(1024 + (n % 60000) as u16));
    }

    let report = engine(paper.then_stop())
        .from(Tcp)
        .extract(Sni)
    ...
    assert!(
        report.forgotten() > 0,
        "разговоров было больше потолка — движок обязан сказать, скольких он перестал видеть \
         живыми; сказано: {}",
        report.forgotten()
    );
}
