...
impl IntoCarrier for Nfqueue {
    ...
    fn open(self) -> Result<NfqueueCarrier, Cause> {
        // ДВА ПИСАТЕЛЯ ОДНОЙ ОБЛАСТИ — прежде всего прочего, и отказом-значением: область марки под
        // признак своего впрыска и раскладка приборов живут в ОДНОМ СЛОВЕ (в марке пакета: память
        // прибора едет вердиктом обоими полями, `CtMark` и `SkbMark`, — на урезанном netfilter
        // прямой записи `ct mark` нет). Вопрос «в одном ли слове» здесь не праздный: `Region` слова
        // не знает, и сравнение областей из разных слов дало бы ложную тревогу — см. `core::mark`,
        // «область без слова — половина высказывания». А пересечение их
        // невыразимо ничем, кроме масок (`core::mark`, «объявить владение вместо того, чтобы о нём
        // помнить»). Раскладку задаёт потребитель (`Nfqueue::marking`), и его числа наших не знают
        // — проверить обязаны мы, до того как приборы начнут писать поверх метки впрыска.
        let ours = reflex_core::mark::Region::new(self.layout.mask())
            .ok_or_else(|| Cause("раскладка приборов не есть связная область".to_string()))?;
        if reflex_core::mark::injecting().overlaps(&ours) {
            return Err(Cause(format!(
                "раскладка приборов ({:#010x}) пересекается с областью метки впрыска ({:#010x}):                  два писателя одних битов затрут друг друга молча — задай другую маску в `marking`",
                ours.mask(),
                reflex_core::mark::injecting().mask(),
            )));
        }
