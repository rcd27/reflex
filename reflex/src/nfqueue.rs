...
impl Nfqueue {
    ...
    ///
    /// Спрашивается о ЛЮБОЙ очереди, не только о своей: слот поднимает чужой процесс, а знать о
    /// нём нужно нам. Оттого функция, а не метод рецепта, — предмет принадлежит очередям ядра, а
    /// не нашему носителю.
    ///
    ...
    ///
    /// Движок десинка умер при старте на всех коробках, а продукт сутки печатал «нога несёт»:
    /// свидетельством считался успешный `spawn`, то есть ОБЪЯВЛЯЮЩИЙ. Ядро знало правду всё это
    /// время — строка очереди есть ровно тогда, когда к ней привязан сокет.
    ///
    ...
    ///   движок). Ответ «слушают» не есть ответ «слушает твой».
