...
//!
//! Замер 14.09.2026 на боевой машине: прибор темпа объявил троттлингом служебные соединения PSN, их
//! увод в контур выкинул человека из игры. Проверить прибор можно только прогоном ПОЛЕВОЙ записи
//! через тот же движок, а `tcpdump` на машине нет — и смотрел бы он другим глазом: на мост до
...
//!
//! Час TCP машины — около 1,2 ГБ, и почти весь объём составляют тела ответов (видео). Содержимое
...
//!
//! Хвост текущего поколения живёт в буфере до 8 КБ: файл, снятый с машины на ходу, может кончаться
...
impl Record {
    /// Запись по пути. Умолчание — четыре поколения по 32 МБ (около часа TCP машины), цель на 443.
