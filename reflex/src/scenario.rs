...
impl Paper {
    ...
    /// перешагнутый узел. На живом носителе это случается, когда приборы медленнее сетки: срок уже прошёл,
...
impl Serves for Paper {
    /// Край ПО СЦЕНАРИЮ (`.edging(...)`), а не по сообщению: `Message` его не носит — край живёт
    /// на `Serves`. Оттого подмена края декоратором наблюдаема ЗДЕСЬ, в бумажном носителе, а не
    /// только в цели.
    type Edge = PaperEdge;
