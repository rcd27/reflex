...
impl<S> Iterator for Chorus<S> {
    ...
    fn next(&mut self) -> Option<Note<S>> {
        loop {
            ...
            // в нашем сне, и приходит оно коротким ломтем, а не полным сроком: соседка, у которой
            // появится работа, ждёт не дольше ломтя.
            let at = self.next % self.live.len();
            self.next = at + 1;
            if !self.live[at].turn(&mut self.said, Instant::now() + SLICE) {
                self.live.remove(at);
                self.next = at;
            }
        }
