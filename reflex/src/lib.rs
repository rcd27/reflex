...
impl<C: Bordered, T: Transport, H: MarkHome, S> Detecting<C, T, H, S> {
    ...
    /// мира).
    ///
    /// КТО ГОВОРИТ БЕЗ КРАЯ — замерено, а не выведено из имени прибора, и первая редакция этого
    /// абзаца тут ошиблась: она называла приборами узла `Silence` и `Choked`. `Silence` ставит ПАРУ
    /// (`Placed::Both`), и краевая её половина (`EdgeSilence`) шагает ТОЛЬКО на букве `Packet` —
    /// ct-вид едет с пакетом, — то есть её слова всегда С КРАЕМ. Без края приходит слово ПРОВОДНОЙ
    /// машины, сказанное на узле; предъявлено прогоном в `examples/sever-silent-drop`
    /// (`a_word_spoken_on_a_grid_node_arrives_without_an_edge`), где такая машина написана явно.
    ///
