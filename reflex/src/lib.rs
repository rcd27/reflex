...
pub trait Transport {
    ...
    /// (2) буква `Opaque { why: Truncated }` с боевого пути не рождается вовсе, и работа пяти
...
fn name_from_hello(ident: &mut Ident, wire: &reflex_engine::parse::Wire<'_>) {
    ...
    // Порядок же в бою не обещан никем: голова может быть дропнута до нашего хука и прийти лишь
    // повтором, уже после хвоста.
    //
    // Замер потребителя, на котором это поймано: цель с приветствием в 1380 + 190 байт давала имя
    // на ЗАПИСИ той же беседы и не давала в бою ни разу — все беды приезжали голым адресом. Разбор
...
/// Оплачено прогоном у потребителя, а не чтением у нас: боевая цепочка, собранная из всего, что
