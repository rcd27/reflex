...
pub trait Transport {
    ...
    /// ВИДЕННЫХ четвёрок и не убывали никогда — замер потребителя 18.09.2026 на живых коробках:
...
fn name_from_hello(ident: &mut Ident, wire: &reflex_engine::parse::Wire<'_>) {
    ...
    // ПОТОЛОК ЗДЕСЬ СТОЯЛ НЕ НА ТОЙ ОСИ, и это стоило памяти на живых коробках. Проверка выше
    ...
    // парой чисел на равном объёме провода — у коробки под ТСПУ 21 повтор приветствия, у
    // канарейки ноль.
    //
...
/// Ядро канарейки (OpenWrt 6.12.71) собрано без `CONFIG_NF_CONNTRACK_TIMESTAMP` — возраста нет,
