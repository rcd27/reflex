...
///
/// Блэкхол сюда же и входит той же прогрессией фаз: `up.packets == 0` — соединения не было;
/// рукопожатие есть, а байтов сверх заголовков нет — цель молчит; байты есть — жива. Одна
/// прогрессия, один прибор; разводить их значило бы завести две фазы над одним разговором.
///
...
///
/// Утв. 7.5 даёт три клетки, в которых тождественное молчание законно; здесь стоит третья —
...
/// `reflex/tests/recording.rs`, мутант без объявления даёт две `NoBytes` подряд).
///
/// ОТДАЁТСЯ СЛОВО ОБЪЯВЛЕНИЕМ ДО ШАГА ([`SilenceInstrument::history_spoken_elsewhere`]), а не
/// фильтром после него, и это не перестановка строк. Фильтр за прибором оставлял автомат в
/// уверенности, что он высказался: `Watch::Fired` тратился на слово, которого никто не слышал, и
/// дверь глохла на ВЕСЬ разговор — всякая цель, чей первый байт опоздал на порог, делала
/// проводную половину немой. Сторож: `a_confiscated_word_does_not_spend_the_doors_one_shot`.
///
...
///
/// ОКНО ПОДТВЕРЖДЕНО ЖИВЫМ ПРОВОДОМ (замер потребителя на живой цензуре, 20.09.2026): при пороге
/// 1600 мс и шаге сетки 200 мс повтор клиента назван через 309 мс от начала запроса, слово двери —
/// через 1932 мс, то есть 1623 мс спустя, внутри `[1600, 1800)`. Прежде закон держался одной лишь
/// переигровкой записи, где сетку двигают наши же часы.
struct Stalling<E> {
