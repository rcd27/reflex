...
impl<C: Bordered, T: Transport, H: MarkHome, S> Detecting<C, T, H, S> {
    ...
    ///
    /// [`Whom::edge`] пуст у всякого слова, рождённого узлом сетки: у времени края не бывает. А
    /// приборы тишины (`Silence`, `Choked`) только на узлах и говорят — значит в предикате вида
    /// `whom.edge.map(…) == Some(МАРКА)` их слова дают `false` ВСЕГДА, и читается это как «чужой
