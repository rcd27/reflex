...
fn name_from_hello(ident: &mut Ident, wire: &reflex_engine::parse::Wire<'_>) {
    ...
    //
    // Прежде основание бралось у ПЕРВОГО клиентского сегмента с данными, каким бы он ни был, — и
    // хвост, пришедший раньше головы, отравлял его навсегда: голова получала сдвиг «назад»
    // (обёртка номера даёт величину около четырёх миллиардов), потолок её отбрасывал, и имя не
    // поднималось НИКОГДА. Порядок же в бою не обещан никем: голова могла быть дропнута до нашего
    // хука и прийти лишь повтором, уже после хвоста.
    //
...
///
/// СЕКУНДА — решение владельца 14.09.2026 («сбрось все хардкоды до 1с»), вопреки замеру выше.
/// Цена названа: вернутся ложные приговоры медленным рукопожатиям. Ловушка судит по трём словам
/// (копредел в продукте), и уводит ложно осуждённую цель в РАБОЧУЮ ногу — крюк, а не потеря.
