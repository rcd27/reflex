...
///
/// * **край** знает историю разговора даром от ядра (сколько пакетов пришло вверх за всё время) и
///   ловит «цель не открыла разговор» (`Blackhole`) и «ответила только `SYN+ACK`» (`NoBytes`).
///   Шагает ТОЛЬКО по пакету: марку на тике не прочесть, ct-вид приезжает с кадром;
...
/// приборов промолчал: краевая половина видит `up_packets >= 2`, считает цель живой и отпускает
/// разговор навсегда. Проводная половина на той же записи называет беду через 0,6 секунды при
/// пороге 500 мс.
///
