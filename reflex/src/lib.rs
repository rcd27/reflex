    ...
    S: Word + Clone + PartialEq + 'static,
{
    ...
    if chain.severing.is_some() {
        return Report::not_started(
            chain.carrier.name(),
            "правило обрыва (`severing`) объявлено, но цепочка идёт терминалом (`on`/`act`):              правило исполняется только на пути показаний. Либо рвите актом              (`.act(|_, _| Act::sever())`), либо снимите терминал и берите показания значением              (`.heard()`, `together()`)"
                .to_string(),
        );
    }
