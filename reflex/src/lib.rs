...
pub use reflex_core::types::{Addr, Dir};
pub use reflex_engine::parse::{Datagram, Read};

/// РАЗОБРАННЫЙ КАДР — комнатой, а не в корне, и слово тому виной: `Wire`.
///
/// В корне оно уже занято смыслом, и другим: [`Transport::Wire`] — ШИРОКИЙ СЛОВАРЬ наблюдений
/// транспорта (`Reading`, `DnsMessage`), то есть то, что видит прибор. `parse::Wire` — РАЗОБРАННЫЙ
/// СЕГМЕНТ, то есть то, из чего наблюдение родилось. Конфликта имён в языке нет (ассоциированный тип
/// живёт под трейтом), а у читателя есть: пишущий свой транспорт объявляет `type Wire = MyVocab;` и
/// тут же разбирает `Read::Tcp(w)`, где `w` — другой `Wire`. Это ровно тот случай, ради которого
/// разведён [`carrier::Observed`] и [`Observed`] цепочки, и решается он так же — комнатой.
///
/// Столкновение нашёл потребитель, а не мы: у него свой `Wire<P: Alphabet>`, и `use reflex::*`
/// принёс бы второе имя в ту же область. Починено до того, как он поднял подмодуль, — потом было бы
/// дороже ему, а не нам.
pub mod frame {
    pub use reflex_engine::parse::{Ends, Head, Header, Wire};
}
