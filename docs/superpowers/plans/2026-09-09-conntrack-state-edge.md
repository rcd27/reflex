...
### Task 11: Снос юзерспейсного состояния — ЗАКРЫТА ЦЕЛЬЮ, не работой

**Итог по коду (18:30).** `FlowTable` в дереве имеет ровно одного потребителя — фасад
(`reflex/src/lib.rs`, две петли). Ядерный путь (`examples/detect-silent-drop/src/edge_main.rs`)
её не касается вовсе: он бежит `EdgeSilence` по пакетам `QueueSocket`, состояние держит ct-марка.
То есть **юзерспейс zero-state достигнут там, где мы его строили**, и убирать из ядерного пути
нечего — он table-free с рождения.

Для фасада же «снять таблицу» = ПЕРЕЕЗД, а не снос, и по двум причинам, обе в коде:

1. Приборы `instrument/src/detect.rs` на край не переведены. `EdgeSilence` — их параллельная
   замена, а не миграция: старые полны живого пофлоу-состояния и все в ходу у фасада.
2. Тишина фасада живёт ТИКАМИ (`table.tick(now)`). Один экземпляр без ключа смешал бы потоки, а
   без тиков часы молчания встали бы — детекция умерла бы. Чтобы приборы стали stateless, их
   состояние обязано уехать в ядро, а ct-марку несёт `QueueSocket`, не `NfqueueBackend`.

Переезд фасада — отдельное решение rcd (смена носителя плюс перевод парка приборов), не часть
этого плана. Пока он не принят, в дереве сосуществуют два понимания тихого дропа: пофлоу-автоматы
фасада и ядерный прибор. Это осознанная цена сравнения, и она же — открытый шов: два закона об
одном предмете расходятся молча.
