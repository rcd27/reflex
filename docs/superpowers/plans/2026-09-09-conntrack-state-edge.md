...
### Task 11: Снос юзерспейсного состояния — ЗАКРЫТА ПЕРЕЕЗДОМ (10.09.2026)

**Итог по коду.** Переезд состоялся: фасад целиком живёт на `QueueSocket`, состояние приборов,
чей предмет виден ядру, уехало в марку. Запись ниже (от 18:30 09.09) описывала мир ДО решения rcd
и сохранена как история — она верно называла, почему снос невозможен без переезда.

Что сделано вместо сноса:

* **Одна дверь, два дома.** Широкое слово стало парой `(Wire, Option<CtEdge>)`: провод и край
  говорят об ОДНОМ разговоре, значит область одна (§5 разводит области, а не источники). Проводные
  приборы сужают к своему алфавиту через `Reads`, краевые читают пару целиком.
* **Род объявляет прибор**, не потребитель (`IntoProbe::place`). `.detect(X)` не изменился, а петля
  гонит приборы по-разному: проводной — копией на ключ с тиками, краевой — одним экземпляром без
  тиков (его часы приезжают величиной, `age` conntrack).
* **Разбиение по видимости предмета ядру**, а не «zero-state»: `Silence` и `SynDrop` — на край
  (обе буквы даёт один `EdgeSilence`: `up.packets` 0 → блэкхол, 1 → тишина, ≥2 → цель жива);
  `Retransmit` (`seq`) и `DnsPoison` (содержимое) остались в проводе, ибо ядро их предмета не видит.
* **`FlowTable` жива и честна** — она носитель предметов, невидимых ядру, а не недоснесённый долг.

**Шов закрыт.** Двух пониманий тихого дропа в фасаде больше нет: тишина одна, краевая. Второй путь
(`edge_main.rs`, очередь 201) остаётся независимым свидетелем A/B, а не параллельным законом.

**Подтверждено боем** (docker, реальный ТСПУ): фасад на `rutracker.org` — 20 подозрений от провода,
20 подтверждений от края, 5 слов о цели; `vk.com` чист. `.act` на новом носителе оборвал тихий дроп
RST за 437 мс (без нас соединение висело бы ~12 с).

**Цена названа:** краевой блэкхол судит по возрасту потока (2 с), проводной судил по повтору `SYN`
(сотни мс) — ранняя реакция ушла вместе с переездом.

### Исходная формулировка задачи (для истории)
...
- [ ] **Step 2: Снять таблицу ИЗ ПУТИ ПРИБОРОВ — но не удалять тип**

`FlowTable::<Probes, FlowKey>` уходит из фасадного пути: приборы края состояния не держат и живут одним экземпляром. `idents` **остаётся** — имя цели не состояние автомата.

**Сам тип `FlowTable` НЕ удаляется, и это не осторожность, а следствие того, что впереди.** §12.1 канона называет её первым шагом замыкания: «`FlowTable` обобщается по ключу и становится `Step`». Ключевание из цепочки (см. раздел «Куда это ведёт» в спеке) даёт области, чей носитель юзерспейсный — цель по имени в ядре не живёт и жить не может. Для таких ключей таблица и понадобится. Удалить её сейчас значило бы снести то, что через задачу придётся писать заново, — и написать хуже, потому что опыт уйдёт вместе с кодом.

**Старый фасадный путь остаётся в дереве рядом.** Снос работающей детекции — точка невозврата, и решение о ней принимает rcd отдельно, а не план. До тех пор два пути живут вместе: это цена сравнения, и она оправдана, пока ядерный путь подтверждён одной серией из двадцати потоков за один день на одном ТСПУ.
