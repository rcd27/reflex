...
> **ПРАВКИ ПО ХОДУ ИСПОЛНЕНИЯ (06.09.2026).** Код ниже — редакция ДО ревью; в дереве он другой,
> и разошёлся он по двум решениям контроллера, каждое из которых записано в сообщении своего
> коммита:
>
> * **`d81a79a`** — третий тест `machine_outlives_the_borrowed_packet` УДАЛЁН: он утверждал то,
>   чего тип не умеет (`E0597`). Находка шире теста: `Step` с обычным `type From` не выражает
>   вход, заимствованный только на время вызова. Названо в докблоке, несётся к плану №4.
> * **`841bcdf`** — поле `epoch` из `Advancing` УБРАНО, эпоха берётся у буквы (`plan.epoch`):
>   с полем машина расходилась с `Plane::feed` ровно на смене плана посреди разговора. Докблок
>   про ограничение переписан — первая редакция переусиливала («ровно одного заимствования»)
>   и винила ковариантность, тогда как опровергнут был вывод из неё.
>
> Ожидание шага 4 — **два** теста, не три. Верен код в дереве, а не блок ниже.
