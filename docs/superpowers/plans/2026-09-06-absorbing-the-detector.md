...
### Живость семи комбинаторов — мерена у ПОТРЕБИТЕЛЯ, не у себя

В `reflex/*/src` у всех семи **ноль** вызовов. Для приложения это приговор, для **фреймворка — нет**: его API употребляет потребитель, живущий вне репы.

| комбинатор | у потребителя | судьба |
|---|---|---|
| `.and` | 35 | портируется |
| `.lmap` | 8 | портируется |
| `.timed` | 6 | портируется |
| `.contextual` | 4 | портируется |
| `.rmap` | 2 | портируется |
| `.by` | **0** | сносится (задача 1) |
| `.changes` | **0** | сносится (задача 1) |
...
## Задача 1: пять комбинаторов переезжают в `Step`, два мёртвых сносятся

**Files:**
- Modify: `core/src/detector.rs` — комбинаторы `Both`, `RMap`, `LMap`, `Contextual`, `Timed`; снос `By`/`Told`/`Changes`
- Modify: `core/src/step.rs` — методы-комбинаторы в `StepExt`
...
- [ ] **Шаг 1: перечислить поимённо, что сносится**

Снос по инвентарю — правило Global Constraints. Выполнить и сохранить вывод, он идёт в сообщение коммита:

```bash
cd /home/rcd/Workspace/reflex
grep -n "^fn \|^async fn " core/tests/detector_combinators.rs
grep -rn "\.by(\|\.changes()\|By<\|Told<\|Changes<" --include=*.rs core engine engine-nfq instrument runtime
```

Ожидается: 20 тестовых функций в файле комбинаторов; вхождения `by`/`changes`/`Told` только внутри `core/src/detector.rs` и `core/tests/detector_combinators.rs`. **Если вхождение найдётся где-то ещё — остановиться и доложить: замер устарел.**
...
- [ ] **Шаг 9: снести `By`, `Told` и `Changes`**

Удалить из `core/src/detector.rs`: `pub struct Told<S>` с докблоком, `pub struct By<D>`, `impl<D: Detector> Detector for By<D>`, `impl<D: Clone> Clone for By<D>`, `pub struct Changes<D: Detector>` с докблоком, `impl<D> Detector for Changes<D>`, а также `Clone` для `Changes`, если он есть.

Удалить из `core/src/lib.rs` строки 54-56 имена `By`, `Changes`, `Told` из списка реэкспорта.

**Основание сноса — замер, а не вкус:** у потребителя ноль вызовов обоих.

**И названная потеря.** Докблок `Told` говорит: «`and` складывает наблюдателей в один поток, и без имени два прибора с общим словарём (`Silence` и `Choked` оба говорят "байтов нет") дают неразличимые показания при разном лечении.» Это лекарство от неотличимости — беды, которую vision называет 63 раза. Снос лекарства, которого никто не принимает, — не отказ от лечения, а признание, что **необязательное лекарство не лечит**. Атрибуция обязана вернуться ОБЯЗАТЕЛЬНОЙ, частью выходного слова (спека §2: `Out = (word, observations)`), а не необязательным комбинатором. Записать это в докблок модуля `core/src/detector.rs` отдельным абзацем — как долг, а не как сожаление.

- [ ] **Шаг 10: перенести методы в `StepExt`**

В `core/src/step.rs`, внутрь `pub trait StepExt`, добавить пять методов. Границы называют форму алфавита там же, где употребляются:
...
**`DetectorExt` НЕ удаляется в этой задаче** — трейт `Detector` ещё жив и его реализуют 42 типа; удаление ext-трейта сейчас ничего не ломает, но и ничего не даёт. Удаляются только пять методов, переехавших в `StepExt`, а `by`/`changes` — вместе со своими типами (шаг 9). После этого `DetectorExt` остаётся пустым трейтом и сносится в задаче 3 вместе с `Detector`.
...
3. Удалить **шесть** тестов, сторожащих снесённое, — четыре на `changes` и два на `by`: `changes_says_it_once_however_often_it_is_seen`, `changes_reports_a_return_to_a_previous_reading`, `changes_is_idempotent`, `changes_lets_the_very_first_reading_through`, `attribution_survives_composition`, `attribution_changes_nothing_but_the_name`. Замерено 06.09.2026: `awk '/^fn /{n=$2} /\.by\(|\.changes\(|Told/{print n}' core/tests/detector_combinators.rs | sort -u` даёт ровно эти шесть. **Перед удалением каждого — прочитать его тело и убедиться, что он сторожит ТОЛЬКО снесённое.** Тест, употребляющий `by`/`changes` мимоходом ради проверки чего-то другого, — переписывается, а не сносится. Это правило Global Constraints, и на прошлой ветке его нарушение стоило трёх тестов закона 3.
4. Остальные **четырнадцать** (20 было, 6 снесено) переписать под новые имена методов; тела ассертов не менять.
...
Сообщение обязано отчитаться **за каждый снесённый пункт** из инвентаря шага 1 — сколько тестов было, сколько снесено, сколько переписано, и что сторожил каждый снесённый.
...
* **Не возвращает атрибуцию.** `by`/`Told` сносятся, долг записан: атрибуция обязана вернуться обязательной частью выходного слова, а не комбинатором, который никто не зовёт.
