...
impl<T: Clone + PartialEq> Subject<T> {
    /// `set`, подавляющий ПОВТОР: подписчик будится, только если значение сменилось (Rx
    /// `distinct_until_changed`). Отдельный метод, а не поведение `set`: bound здесь уже, чем `T:
    /// Clone` у `Subject` (потребителю `Subject<Arc<…>>` сравнивать значения нечем, глобальный bound
    /// отобрал бы примитив целиком) — метод существует ровно там, где есть чем сравнивать. Сравнение
    /// и запись под ОДНИМ write-замком: с двумя два писателя сверяются с одним снимком, оба видят «не
    /// изменилось», и уведомление о промежуточном значении теряется.
