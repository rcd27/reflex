...
impl<T: Clone + PartialEq> Subject<T> {
    ...
    /// `Subject`: живой потребитель `Subject<Arc<Знание>>` (`nevod/src/ladder/verdict_table.rs`)
    ...
    /// флоу (`nevod/src/ladder/flow_registry.rs`) меняется на КАЖДОМ born/died — повтор там даёт
