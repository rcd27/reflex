...
//! часы не нужны, и это не выбор: шов, берущий [`reflex_core::clock::Clock`] и штампующий сам, не
//! собирается — поток тиков привязан к заимствованию часов. Отказ компилятора и называет закон:
//! способ ожидания есть свойство того, кто ПОРОЖДАЕТ поток, а не сшивает. Оттого два потока и ни одних
