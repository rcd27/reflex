...
pin_project! {
    ...
    ///
    /// # ЗАКОН ВХОДА: ЧЕРЕЗ ЭТОТ ПОДЪЁМ ЕДУТ ДВЕ БУКВЫ ИЗ ТРЁХ
    ///
    /// Оператор одевает всякий элемент источника в [`DetectorEvent::Packet`], а узлы своей сетки —
    /// в `Tick`. Третьей буквы, `Opaque`, он не строит НИ ПРИ КАКОМ входе: источник отдаёт ему
    /// разобранное значение, и сказать «пришло, но разобрать не смогли» здесь уже некому — разбор
    /// случился раньше, до потока.
    ///
    /// Значит непонятое входит в цепочку ДРУГОЙ дверью — швом ([`crate::timed::on_grid`]), где обе
    /// буквы сходятся на одну сетку. Шаг, поднятый здесь, ветку `Opaque` всё равно обязан
    /// разобрать: дверей у алфавита больше одной, и шаг не выбирает, которой его поднимут.
